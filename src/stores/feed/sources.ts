...
export const createFeedSourcesState = ({
	...
	const requestEntitySubscription = async (
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscribe/${entity}/${id}`,
				{
					method,
			...
			reportError(
				e,
				`Failed to ${method === "POST" ? "subscribe to" : "unsubscribe from"} ${entity}`,
			);
			return false;
		}
	};
	...
	const subscribeToChannel = (channelId: string) =>
		requestEntitySubscription("channel", channelId, "POST");
	const unsubscribeFromChannel = (channelId: string) =>
		requestEntitySubscription("channel", channelId, "DELETE");
	const subscribeToCollection = (collectionId: string) =>
		requestEntitySubscription("collection", collectionId, "POST");
	const unsubscribeFromCollection = (collectionId: string) =>
		requestEntitySubscription("collection", collectionId, "DELETE");

	const isSubscribedToEntity = async (
		entity: "channel" | "collection",
		id: string,
	): Promise<boolean> => {
		const authStore = useAuthStore();
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscribe/${entity}/${id}/status`,
				{
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
			);
	...
	const isSubscribedToChannel = (channelId: string) =>
		isSubscribedToEntity("channel", channelId);
	const isSubscribedToCollection = (collectionId: string) =>
		isSubscribedToEntity("collection", collectionId);

	const subscribeToRSS = async (
		rssUrl: string,
		title?: string,
	): Promise<boolean> => {
		const authStore = useAuthStore();
	...
	const addSubscription = async (payload: {
		...
		try {
			...
			if (payload.group_id && subscriptionId) {
				const moveRes = await apiRequestResult(
					`${api.url}/feed/subscriptions/${subscriptionId}/group`,
					{
						method: "PUT",
						headers: {
							"Content-Type": "application/json",
	...
	const discoverFeedCandidates = async (
		url: string,
		...
		try {
			...
			if (!res.ok) {
				const data = res.data;
				if (isCurrentSession(generation))
					error.value = apiErrorMessage(data, "发现订阅源失败");
				return [];
			}
			const data = res.data;
	...
	const resolveSubscriptionInput = async (
		input: string,
	): Promise<ResolvedSubscriptionInput | null> => {
		const authStore = useAuthStore();
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/resolve`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const autoAddSubscription = async (
		payload: AutoAddSubscriptionPayload,
	): Promise<boolean> => {
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated) return false;
		const generation = sessionGeneration;
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/auto-add`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const batchSubscribeSources = async (
		sourceIds: string[],
	): Promise<{
		created: number;
		reusedIds: string[];
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/sources/batch-subscribe`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const importOPML = async (
		file: File,
	): Promise<FeedOPMLImportResult | null> => {
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated) return null;
		const generation = sessionGeneration;
		error.value = null;
		...
		try {
			...
			if (!res.ok) {
				const data = res.data;
				if (isCurrentSession(generation))
					error.value = apiErrorMessage(data, "导入 OPML 失败");
				return null;
			}
	...
	const exportOPML = async (): Promise<Blob> => {
		...
		if (!res.ok) {
			throw new Error(
				apiErrorMessage(await readJsonErrorPayload(res), "导出 OPML 失败"),
			);
		}
	...
	const createSubscriptionFromProvider = async (payload: {
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/sources/create-from-provider`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			if (!res.ok) {
				const data = res.data;
				if (isCurrentSession(generation))
					error.value = apiErrorMessage(data, "创建来源失败");
				return false;
			}
			const subscriptionId = res.data.data?.id;
			if (!isCurrentSession(generation)) return false;
	...
	const unsubscribeFromRSS = async (rssUrl: string): Promise<boolean> => {
		...
		const generation = sessionGeneration;
		try {
			let subscription = subscriptions.value.find(
				(item) =>
			...
			if (!subscription) {
				await fetchSubscriptions();
				if (!isCurrentSession(generation)) return false;
				subscription = subscriptions.value.find(
					(item) =>
						item.feed_source?.source_type === "external_rss" &&
						normalizeRssUrl(item.feed_source.rss_url || "") === normalized,
				);
			...
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscription.id}`,
				{
					method: "DELETE",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
