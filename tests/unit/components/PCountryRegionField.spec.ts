...
describe("PCountryRegionField", () => {
	it("shows common countries before a continent is selected", async () => {
		...
		await wrapper
			.get('[data-test="artist-nationality-trigger"]')
			.trigger("click");

		const dialogText = wrapper
			.get('[data-test="artist-nationality-dialog"]')
			.text();
	...
	it("selects a country from the chosen continent and emits only the chinese country name", async () => {
		...
		await wrapper
			...
			.trigger("click");
		expect(
			wrapper.get('[data-test="artist-nationality-dialog"]').text(),
		).toContain("选择洲");

		await wrapper
			.get('[data-test="artist-nationality-continent-Asia"]')
			.trigger("click");
		expect(
			wrapper.get('[data-test="artist-nationality-dialog"]').text(),
		).toContain("日本");
		expect(
			wrapper.find('[data-test="artist-nationality-option-法国"]').exists(),
		).toBe(false);

		await wrapper
			.get('[data-test="artist-nationality-option-日本"]')
			.trigger("click");

		expect(wrapper.emitted("update:modelValue")?.[0]).toEqual(["日本"]);
		expect(
			wrapper.find('[data-test="artist-nationality-dialog"]').exists(),
		).toBe(false);
	});
	...
	it("shows the localized current country and highlights its continent and country when reopening the dialog", async () => {
		...
		expect(
			wrapper.get('[data-test="artist-nationality-trigger"]').text(),
		).toContain("日本");
		...
		await wrapper
			...
			.trigger("click");
		expect(
			wrapper.get('[data-test="artist-nationality-continent-Asia"]').classes(),
		).toContain("country-field-continent--active");
		expect(
			wrapper.get('[data-test="artist-nationality-option-日本"]').classes(),
		).toContain("country-field-country--active");
	});
});
