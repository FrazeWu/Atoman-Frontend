...
describe('MusicDescriptionPreview.vue', () => {
  it('shows a three-line preview before expanding overflowing text', async () => {
    const wrapper = mount(MusicDescriptionPreview, {
      props: {
        description: 'A detailed introduction that exceeds the available preview height.',
    ...
    expect(description.textContent).toBe('A detailed introduction that exceeds the available preview height.')
    expect(toggle.attributes('aria-expanded')).toBe('false')
    expect(wrapper.get('#test-description').classes()).not.toContain('is-expanded')
  ...
  it('does not render a collapse control for a short description', async () => {
    ...
    expect(wrapper.find('[data-testid="short-description-toggle"]').exists()).toBe(false)
  })
})
