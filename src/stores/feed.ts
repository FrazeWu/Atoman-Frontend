	...
	try {
		...
		return {
			...
			autoAddReadingListSourceIds: normalizeRuleList(
				parsed.autoAddReadingListSourceIds,
			),
		};
	} catch {
		return { autoMarkReadSourceIds: [], autoAddReadingListSourceIds: [] };
...
const writeAutomationRules = (rules: FeedAutomationRules) => {
	...
	localStorage.setItem(
		FEED_AUTOMATION_RULES_STORAGE_KEY,
...
export const useFeedStore = defineStore("feed", () => {
	...
	const pollInterval: ReturnType<typeof setInterval> | null = null;
	...
	const setFilterRules = (rules: Partial<FeedFilterRules>) => {
		const nextRules: FeedFilterRules = {
			mutedSourceIds: normalizeRuleList(
				rules.mutedSourceIds ?? filterRules.value.mutedSourceIds,
			),
			hiddenKeywords: normalizeRuleList(
				rules.hiddenKeywords ?? filterRules.value.hiddenKeywords,
			),
		};
	...
	const fetchFilterPreferences = async () => {
		...
		try {
			const res = await apiRequestResult(`${api.url}/feed/preferences`, {
			...
			applyFilterRules({
				hiddenKeywords: normalizeRuleList(data.data?.hidden_keywords),
				mutedSourceIds: [],
			});
			return true;
		} catch {
	...
	const setAutomationRules = (rules: Partial<FeedAutomationRules>) => {
		const nextRules: FeedAutomationRules = {
			autoMarkReadSourceIds: normalizeRuleList(
				rules.autoMarkReadSourceIds ??
					automationRules.value.autoMarkReadSourceIds,
			),
			autoAddReadingListSourceIds: normalizeRuleList(
				rules.autoAddReadingListSourceIds ??
					automationRules.value.autoAddReadingListSourceIds,
			),
		};
	...
	const fetchTimeline = async (
		...
		try {
			const options: FeedTimelineFetchOptions =
				typeof sourceTypeOrOptions === "object" && sourceTypeOrOptions !== null
					? sourceTypeOrOptions
					: {
							sourceType:
			...
			const url = query
				? `${api.url}/feed/timeline?${query}`
				: `${api.url}/feed/timeline`;
	...
	const markSubscriptionRead = async (
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscriptionId}/mark-read`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const markSubscriptionUnread = async (
		subscriptionId: string,
	): Promise<boolean> => {
		const authStore = useAuthStore();
		if (!subscriptionId) return false;
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscriptionId}/mark-unread`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const markAllRead = async (): Promise<boolean> => {
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/timeline/mark-all-read`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const markAllUnread = async (): Promise<boolean> => {
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/timeline/mark-all-unread`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
