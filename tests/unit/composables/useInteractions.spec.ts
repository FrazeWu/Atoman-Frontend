...
describe("useInteractions", () => {
  it("calls module-prefixed like endpoint with target body", async () => {
    fetchMock.mockResolvedValueOnce(
      response({ data: { Liked: true, LikeCount: 2 } }),
    );
  ...
  it("uses unified Forum discussion comments and topic like endpoints", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/api/v1/discussions/forum_topic/topic-1/comments",
      expect.anything(),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/v1/forum/topics/topic-1/like",
  ...
  it("adds a short-note reply from the create response without refetching the thread", async () => {
    ...
    expect(interactions.comments.value[0]?.replies).toMatchObject([
      { id: "reply-1", content: "reply" },
    ]);
  ...
  it("uses the latest reactive target id", async () => {
    fetchMock.mockResolvedValueOnce(
      response({ data: { Liked: true, LikeCount: 1 } }),
    );
  ...
  it.each([
    ...
    "does not apply a late %s response after the target changes from A to B",
    async (action, method) => {
  ...
  it.each([
    ...
    [
      ...
      "like",
      "POST",
      "/api/v1/forum/topics/topic-a/like",
      false,
      true,
      7,
    ],
    ...
    "does not apply a late %s %s response after the target changes from A to B and back to A",
    async (moduleName, targetType, action, method, endpoint, initialLiked, responseLiked, initialLikeCount) => {
      const lateResponse = deferred<Response>();
      fetchMock.mockReturnValueOnce(lateResponse.promise);
  ...
  it.each([
    ...
    "does not apply an old %s like response after targets cycle from A to B to A",
    async (moduleName, targetType, endpoint) => {
  ...
  it.each([
    ...
    "keeps the latest %s like operation when like and unlike responses finish in reverse order",
    async (moduleName, targetType) => {
  ...
  it("keeps comments from the latest target when stale requests finish later", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/api/v1/discussions/video/video-a/comments",
      expect.anything(),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/v1/discussions/video/video-b/comments",
      expect.anything(),
    );
    expect(interactions.comments.value.map((comment) => comment.id)).toEqual([
      "comment-b",
    ]);
