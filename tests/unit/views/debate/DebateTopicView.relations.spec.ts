...
describe('DebateTopicView relation experience', () => {
  ...
  it('从统一 ContentReference 契约渲染并重新确认辩论引用', async () => {
    const unifiedDebate: Debate = {
      ...root,
      content: `现有证据支持 @debate:${staleID}:support。`,
      references: [{
        id: 'reference-stale', field: 'content', start: 7, end: 61,
        raw: `@debate:${staleID}:support`,
        target: { type: 'debate', id: staleID, label: '吸烟显著增加肺癌风险', module: 'debate', path: `/debate/${staleID}`, available: true },
        relation: { id: 'relation-stale', stance: 'support', state: 'stale', conclusion_event_id: 'conclusion-next', conclusion_direction: 'no' },
      }],
    }
    store.currentDebate = unifiedDebate
    store.fetchDebate.mockResolvedValue(unifiedDebate)
    ...
    expect(content.text()).toContain('吸烟显著增加肺癌风险')
    expect(content.text()).toContain('来源结论已变化')
    await content.get('[data-reconfirm-reference="relation-stale"]').trigger('click')
    await flushPromises()
