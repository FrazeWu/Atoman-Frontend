...
export function useMusicAlbumCoverEditor() {
  ...
  const unresolvedImportedCoverUrl = computed(() => {
    const next = albumImportDraft.value?.coverUrl?.trim() || albumImportDraft.value?.derivedCover?.trim() || ''
  ...
  function reopenImportedCoverCrop() {
    if (unresolvedImportedCoverUrl.value) queueImportedCoverCrop(unresolvedImportedCoverUrl.value)
  }
  ...
  watch(unresolvedImportedCoverUrl, (nextCoverUrl) => {
    if (!creationFlow.value || !nextCoverUrl || albumDetailsDraft.value?.coverUrl.trim()) return
    if (pendingCoverCrop.value?.kind === 'manual' || handledImportedCoverUrl.value === nextCoverUrl) return
    if (pendingCoverCrop.value?.kind === 'imported' && pendingCoverCrop.value.sourceUrl?.trim() === nextCoverUrl) return
  ...
  return {
    ...
    confirmCoverCrop,
    clearPendingCoverCrop: () => { pendingCoverCrop.value = null },
  }
}
