...
export function useMusicAlbumTrackEditor() {
  ...
  function updateTracks(mutator: (tracks: typeof tracksDraft.value) => typeof tracksDraft.value) {
    if (!creationFlow.value) return
    creationFlow.value.tracksCustomized = true
    creationFlow.value.draft.tracks = mutator(creationFlow.value.draft.tracks).map((track, index) => ({
      ...track,
      sequence: index + 1,
    }))
  }
  ...
  function updateTrackTitle(trackId: string, title: string) {
    updateTracks(tracks => tracks.map(track => track.id === trackId ? { ...track, title } : track))
  }
  ...
  function handleTrackDragOver(trackId: string) {
    if (draggedTrackId.value && draggedTrackId.value !== trackId) dragOverTrackId.value = trackId
  }
  ...
  function handleTrackDrop(targetTrackId: string, event: DragEvent) {
    ...
    updateTracks((tracks) => {
      ...
      next.splice(sourceIndex < targetIndex ? targetIndex - 1 : targetIndex, 0, sourceTrack)
      return next
    })
  }

  const removeTrack = (trackId: string) => updateTracks(tracks => tracks.filter(track => track.id !== trackId))
  const openTrackLyrics = (trackId: string) => { lyricTrackId.value = trackId }
  const closeTrackLyrics = () => { lyricTrackId.value = null }

  function saveExistingTrackLyrics(lyrics: MusicSongLyrics) {
    const track = lyricTrack.value
    if (!track?.songId || String(track.songId) !== String(lyrics.song_id)) return
