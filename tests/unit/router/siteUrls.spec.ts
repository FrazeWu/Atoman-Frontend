...
import {
  subdomainDefaultPath,
  channelUrl,
  moduleUrl,
  userUrl,
} from "../../../src/composables/useSubdomainNav";
...
describe("site URL builders", () => {
  ...
  it("builds user and channel URLs as explicit paths", () => {
    ...
    expect(channelUrl("design", "https:", "blog.atoman.org")).toBe(
      "/channels/design",
    );
  });
  ...
  it("uses the same path-only URLs in local development", () => {
    ...
    expect(modulePathUrl("music", "/album/123", "http:", "localhost")).toBe(
      "/music/album/123",
    );
  ...
  it("builds module path URLs with public path segments", () => {
    expect(
      modulePathUrl("music", "/album/123", "https:", "blog.atoman.org"),
    ).toBe("/music/album/123");
  });

  it("does not duplicate an existing module path prefix", () => {
    expect(modulePathUrl("video", "/videos/watch/video-1")).toBe(
      "/videos/watch/video-1",
    );
    expect(modulePathUrl("feed", "/feed/sources?source_id=source-1")).toBe(
      "/feed/sources?source_id=source-1",
    );
    expect(modulePathUrl("timeline", "/timeline#event-1")).toBe(
      "/timeline#event-1",
    );
  });
