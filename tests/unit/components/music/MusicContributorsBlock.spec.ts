...
describe('MusicContributorsBlock', () => {
  it('uses spacing instead of structural borders', () => {
    expect(contributorsSource).not.toMatch(/\.music-contributors\s*\{[\s\S]*?border-(top|bottom):/)
    expect(contributorsSource).toContain('margin-top: 2rem')
  })
