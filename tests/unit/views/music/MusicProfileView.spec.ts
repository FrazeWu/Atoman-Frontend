...
vi.mock("@/components/ui/PEmpty.vue", () => ({
  default: {
    ...
    template:
      '<section data-testid="empty-state"><h2>{{ title }}</h2><p>{{ description }}</p><slot name="action" /></section>',
  },
}));
...
vi.mock("@/components/music", () => ({
  MusicAlbumCard: {
    ...
    template:
      '<article data-testid="album-card" @click="$emit(\'click\')"><span>{{ album.title }}</span><button data-testid="album-bookmark" type="button" @click.stop="$emit(\'toggle-bookmark\')">remove</button></article>',
  },
  MusicArtistCard: {
    props: ["artist"],
    emits: ["click", "toggle-bookmark"],
    template:
      '<article data-testid="artist-card" @click="$emit(\'click\')"><span>{{ artist.name }}</span><button data-testid="artist-bookmark" type="button" @click.stop="$emit(\'toggle-bookmark\')">remove</button></article>',
  },
}));
...
function setSuccessfulResponses() {
  ...
  mocks.getMusicHome.mockResolvedValue({
    recently_played: [{ id: "recent-1" }, { id: "recent-2" }],
  });
}
...
describe("MusicProfileView.vue", () => {
  ...
  it("shows the login state without requesting private music data", async () => {
    ...
    expect(wrapper.find('[data-testid="music-profile-refresh"]').exists()).toBe(
      false,
    );
  ...
  it("loads statistics, renders favorites, opens entities, and removes an album bookmark", async () => {
    ...
    expect(
      wrapper.findAll(".music-profile__stat strong").map((node) => node.text()),
    ).toEqual(["12", "2", "2"]);
    ...
    expect(wrapper.findAll(".music-profile__stat strong").at(1)?.text()).toBe(
      "1",
    );
  });
  ...
  it("shows an error and recovers through the refresh control", async () => {
    ...
    expect(
      wrapper
        .get('[data-testid="music-profile-refresh"]')
        .attributes("disabled"),
    ).toBeUndefined();
