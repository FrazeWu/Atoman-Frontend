...
describe("UI 准则", () => {
	it("移动端顶栏隐藏模块导航并禁止导航文字换行", () => {
		...
		expect(source).toMatch(
			/@media \(max-width: 720px\)[\s\S]*?\.nav\s*\{[\s\S]*?display:\s*none/,
		);
	...
	it("空状态和管理界面不暴露英文内部文案", () => {
		expect(read("src/components/ui/PEmpty.vue")).toContain(
			"kicker: '暂无内容'",
		);
		expect(
			read("src/components/music/MusicSidebarPlaylists.vue"),
		).not.toContain("PLAYLISTS");
		expect(read("src/views/forum/ForumLayout.vue")).not.toMatch(
			/CATEGORIES|TAGS/,
		);
		expect(
			read("src/components/setting/SettingFeedSourcePanel.vue"),
		).not.toContain("方便后台识别");
		expect(
			read("src/components/setting/SettingMusicReviewPanel.vue"),
	...
	it("代表页面不再使用档案纸装饰", () => {
		...
		expect(login).not.toMatch(
			/repeating-linear-gradient|box-shadow:\s*[268]px\s+[268]px\s+0/,
		);
	...
	it("论坛侧栏只显示有内容的分类与标签分组", () => {
		...
		expect(source).toContain(
			'v-if="!sidebarCollapsed && popularTags.length > 0"',
		);
	});

	it("Studio 图标按钮具有可访问名称且页面不嵌套装饰卡片", () => {
		const collections = read(
			"src/components/studio/StudioCollectionManager.vue",
		);
		...
		expect(collections).toMatch(
			/v-if="!collection\.is_default"[\s\S]*?:aria-label="`删除\$\{collection\.name\}`"/,
		);
		expect(channels).toMatch(/:aria-label="`删除\$\{channel\.name\}`"/);
		for (const source of [collections, channels]) {
