...
export type OAuthPendingInfo = {
  ...
  stage: 'complete_profile' | 'confirm_account'
  email: string
}
...
async function readJSON<T>(response: Response, fallback: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(oauthErrorMessage(payload.code, rawMessage))
  }
  return payload as T
}
...
export async function completeOAuthProfile(username: string): Promise<{ returnTo: string }> {
