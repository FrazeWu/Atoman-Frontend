...
function date(value: unknown): string | undefined {
  ...
  return candidate && !candidate.startsWith('0001-01-01') ? candidate : undefined
}
...
async function loadArtist(apiBase: string, id: string, fetcher: Fetcher) {
  const artist = record(await getJson(`${apiBase}/music/artists/${encodeURIComponent(id)}`, fetcher))
  if (!artist || !text(artist.id) || !text(artist.name)) return undefined
  const name = text(artist.display_name) || text(artist.name)
  const description = plainText(artist.bio, `在 Atoman 查看 ${name} 的音乐资料、专辑与作品。`)
  return content(
...
async function loadAlbum(apiBase: string, id: string, fetcher: Fetcher) {
  const album = record(await getJson(`${apiBase}/music/albums/${encodeURIComponent(id)}`, fetcher))
  if (!album || !text(album.id) || !text(album.title)) return undefined
  const title = text(album.title)
  const artists = names(album.artists)
  const description = plainText(album.description, `在 Atoman 查看专辑《${title}》的曲目与资料。`)
  return content(
...
async function loadSong(apiBase: string, id: string, fetcher: Fetcher) {
  const detail = record(await getJson(`${apiBase}/music/songs/${encodeURIComponent(id)}`, fetcher))
  ...
  return content(
    ...
    description,
    {
      ...
      byArtist: artists.map(name => ({ '@type': 'MusicGroup', name })),
      inAlbum: text(album?.title) ? { '@type': 'MusicAlbum', name: text(album?.title) } : undefined,
    },
...
async function loadForumTopic(apiBase: string, id: string, fetcher: Fetcher) {
  const topic = record(await getJson(`${apiBase}/forum/topics/${encodeURIComponent(id)}`, fetcher))
  if (!topic || !text(topic.id) || !text(topic.title)) return undefined
  const title = text(topic.title)
  const description = plainText(topic.content, `在 Atoman 论坛参与“${title}”的讨论。`)
  ...
  return content(
    ...
    description,
    {
      ...
      headline: title,
      text: description,
      author: text(author?.display_name) || text(author?.username)
        ? { '@type': 'Person', name: text(author?.display_name) || text(author?.username) }
        : undefined,
...
async function loadDebate(apiBase: string, id: string, fetcher: Fetcher) {
  const debate = record(await getJson(`${apiBase}/debate/topics/${encodeURIComponent(id)}`, fetcher))
  if (!debate || !text(debate.id) || !text(debate.title)) return undefined
  const title = text(debate.title)
  const description = plainText(debate.description || debate.content, `查看辩题“${title}”的观点、论证与讨论。`)
...
function podcastEpisodeContent(episode: Record<string, unknown>, id: string) {
  ...
  const description = plainText(post?.description || post?.excerpt || post?.content, `收听播客单集《${title}》。`)
  const image = text(episode.episode_cover_url) || text(post?.cover_url) || text(channel?.cover_url)
  return content(
    ...
    description,
    {
      ...
      partOfSeries: text(channel?.name) ? { '@type': 'PodcastSeries', name: text(channel?.name) } : undefined,
    },
    image || undefined,
...
async function loadPodcastEpisode(apiBase: string, id: string, fetcher: Fetcher) {
  const episode = record(await getJson(`${apiBase}/podcast/episodes/${encodeURIComponent(id)}`, fetcher))
  return episode ? podcastEpisodeContent(episode, id) : undefined
}

async function loadPodcastShow(apiBase: string, slug: string, fetcher: Fetcher) {
  const data = record(await getJson(`${apiBase}/podcast/channels/${encodeURIComponent(slug)}/episodes`, fetcher))
  ...
  const description = plainText(channel.description, `收听 ${name} 的播客节目与最新单集。`)
  return content(
...
async function loadVideo(apiBase: string, id: string, fetcher: Fetcher) {
  const video = record(await getJson(`${apiBase}/videos/${encodeURIComponent(id)}`, fetcher))
  if (!video || !text(video.id) || !text(video.title)
    || video.visibility !== 'public' || video.status !== 'published') return undefined
  const title = text(video.title)
  const description = plainText(video.description, `在 Atoman 观看视频《${title}》。`)
  ...
  return content(
    ...
    description,
    {
      ...
      creator: text(channel?.name) ? { '@type': 'Organization', name: text(channel?.name) } : undefined,
    },
...
async function loadVideoCollection(apiBase: string, id: string, fetcher: Fetcher) {
  const videos = array(await getJson(`${apiBase}/videos?collection_id=${encodeURIComponent(id)}`, fetcher))
    .map(record)
    .filter((video): video is Record<string, unknown> => Boolean(video)
      && video?.visibility === 'public' && video?.status === 'published')
  if (!videos.length) return undefined
  const collection = record(videos[0].collection) || array(videos[0].collections).map(record).find(Boolean)
  if (!collection || !text(collection.id) || !text(collection.name)) return undefined
  const name = text(collection.name)
  const description = plainText(collection.description, `浏览视频合集“${name}”。`)
  return content(
...
export async function resolvePublicContentSeo(
  ...
  const routes: Array<[RegExp, (value: string) => Promise<PublicContentSeo | undefined>]> = [
    ...
    [/^\/podcasts\/show\/([^/]+)$/, slug => loadPodcastShow(apiBase, slug, fetcher)],
    [/^\/podcasts\/episode\/([^/]+)$/, id => loadPodcastEpisode(apiBase, id, fetcher)],
    [/^\/videos\/watch\/([^/]+)$/, id => loadVideo(apiBase, id, fetcher)],
    [/^\/videos\/collections\/([^/]+)$/, id => loadVideoCollection(apiBase, id, fetcher)],
  ]
  for (const [pattern, loader] of routes) {
    const match = path.match(pattern)
    if (!match) continue
    try {
      return { matched: true, content: await loader(decodeURIComponent(match[1])) }
    } catch {
...
function cleanDefaultMetadata(html: string) {
  return html
    ...
    .replace(/\s*<(?:meta|link|script)[^>]*(?:data-default-meta|data-page-meta="content")[^>]*>(?:[\s\S]*?<\/script>)?/gi, '')
}
...
export function buildPublicContentHtml(html: string, item: PublicContentSeo) {
  ...
  return cleanDefaultMetadata(html).replace(/<\/head>/i, `    ${tags}\n  </head>`)
}
...
export function buildMissingPublicContentHtml(html: string) {
  ...
  return cleanHtml.replace(/<\/head>/i, '    <meta name="robots" content="noindex, nofollow">\n  </head>')
}

function list(payload: unknown): Record<string, unknown>[] {
  return array(unwrap(payload)).map(record).filter((value): value is Record<string, unknown> => Boolean(value))
}

async function fetchList(apiBase: string, path: string, fetcher: Fetcher) {
  const response = await fetcher(`${apiBase}${path}`, { headers: { Accept: 'application/json' } })
  if (!response.ok) throw new Error(`Public sitemap source unavailable: ${path}`)
  return list(await response.json())
}

export async function collectPublicSitemapItems(apiBase: string, fetcher: Fetcher = fetch): Promise<SitemapItem[]> {
  ...
  const fulfilled = (index: number) => sources[index].status === 'fulfilled'
  ...
  artists.forEach(artist => {
    if (text(artist.id)) items.push({ path: `/music/artist/${encodeURIComponent(text(artist.id))}`, last_modified: date(artist.updated_at) })
  })
  albums.forEach(album => {
    if (text(album.id)) items.push({ path: `/music/album/${encodeURIComponent(text(album.id))}`, last_modified: date(album.updated_at) })
    array(album.songs).map(record).filter(Boolean).forEach(song => {
      if (text(song?.id)) items.push({ path: `/music/song/${encodeURIComponent(text(song?.id))}`, last_modified: date(song?.updated_at) || date(album.updated_at) })
    })
  })
  topics.forEach(topic => {
    if (text(topic.id)) items.push({ path: `/forum/topic/${encodeURIComponent(text(topic.id))}`, last_modified: date(topic.updated_at) })
  })
  debates.forEach(debate => {
    if (text(debate.id)) items.push({ path: `/debate/${encodeURIComponent(text(debate.id))}`, last_modified: date(debate.updated_at) })
  })
  episodes.forEach(episode => {
    if (text(episode.id)) items.push({ path: `/podcasts/episode/${encodeURIComponent(text(episode.id))}`, last_modified: date(episode.updated_at) })
    const channel = record(episode.channel)
    if (text(channel?.slug)) items.push({ path: `/podcasts/show/${encodeURIComponent(text(channel?.slug))}`, last_modified: date(channel?.updated_at) })
  })
  videos.forEach(video => {
    if (text(video.id) && video.visibility === 'public' && video.status === 'published') {
      items.push({ path: `/videos/watch/${encodeURIComponent(text(video.id))}`, last_modified: date(video.updated_at) })
      const collections = [record(video.collection), ...array(video.collections).map(record)].filter(Boolean)
      collections.forEach(collection => {
        if (text(collection?.id)) items.push({ path: `/videos/collections/${encodeURIComponent(text(collection?.id))}`, last_modified: date(collection?.updated_at) })
      })
    }
  })
  return items
