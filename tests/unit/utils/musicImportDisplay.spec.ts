...
describe('music import album display', () => {
  it('prefers the final album title and never uses a track or file as the title', () => {
    expect(musicImportAlbumTitle(importRecord({
      ...
      derivedTracks: [{ title: 'Wake Up Mr. West', audioKey: '', origin: 'tag' }],
  ...
  it('deduplicates unresolved retries for the same artist and album title', () => {
    const records = uniqueMusicAlbumImports([
      importRecord({ importId: 'latest-retry', artistId: 'artist-1', albumTitle: 'GNX' }),
      importRecord({ importId: 'older-retry', artistId: 'artist-1', albumTitle: ' gnx ' }),
      importRecord({ importId: 'other-album', artistId: 'artist-1', albumTitle: 'DAMN.' }),
    ])

    expect(records.map((item) => item.importId)).toEqual(['latest-retry', 'other-album'])
  })
  ...
  it('keeps unresolved imports separate when artist or title is unavailable', () => {
    ...
    expect(records.map((item) => item.importId)).toEqual(['missing-artist-1', 'missing-artist-2'])

    const retried = uniqueMusicAlbumImports([
      importRecord({ importId: 'archive-latest', albumTitle: 'GNX', archiveName: 'GNX.zip', status: 'needs_attention' }),
      importRecord({ importId: 'archive-older', albumTitle: 'gnx', archiveName: 'gnx.zip', status: 'needs_attention' }),
    ])
