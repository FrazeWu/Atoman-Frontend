...
describe('timeline revision proposal API', () => {
  ...
  it('uses typed target and decision routes', async () => {
	...
	await timelineRevisionProposalApi.list('event', 'event 1', 1, 20)
	expect(fetchMock.mock.calls[0]?.[0]).toBe('/api/v1/timeline/events/event%201/revision-proposals?page=1&page_size=20')
