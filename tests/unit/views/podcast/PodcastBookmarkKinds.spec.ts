...
import PodcastFavoritesView from '@/views/podcast/PodcastFavoritesView.vue'
...
describe('podcast bookmark kinds', () => {
  ...
  it('sends the selected kind when favoriting or adding an episode to listen later', async () => {
    ...
    const wrapper = mount(PodcastEpisodeView, {
      global: {
        plugins: [router],
        stubs: {
          PPress: { props: ['label'], emits: ['click'], template: '<button @click="$emit(\'click\')">{{ label }}</button>' },
          PodcastShownotes: true,
          PodcastCommentSection: true,
        },
      },
    })
    await flushPromises()
    ...
    await wrapper.findAll('button').find(button => button.text() === '稍后听')!.trigger('click')
    await flushPromises()
  ...
  it('loads the episode and listen later tabs with their respective kind filters', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => response({ data: [] }))
  ...
  it('keeps the later response when returning to the same tab', async () => {
    ...
    await wrapper.findAll('button').find(button => button.text() === '稍后听')!.trigger('click')
    await flushPromises()
    await wrapper.findAll('button').find(button => button.text() === '单集')!.trigger('click')
    await flushPromises()

    secondEpisodes.resolve(response({ data: [{ id: 'new', episode: { id: 'new', post: { title: '新单集' } } }] }))
    await flushPromises()
    firstEpisodes.resolve(response({ data: [{ id: 'old', episode: { id: 'old', post: { title: '旧单集' } } }] }))
    await flushPromises()
  ...
  it('keeps the current episode bookmarks when reloading returns an HTTP error', async () => {
    vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(response({
        data: [{ id: 'saved', episode: { id: 'saved', post: { title: '已收藏单集' } } }],
      }))
  ...
  it('ignores a stale request failure without changing the current tab data', async () => {
    ...
    secondEpisodes.resolve(response({ data: [{ id: 'current', episode: { id: 'current', post: { title: '当前单集' } } }] }))
    await flushPromises()
