...
describe("Music HistoryView.vue", () => {
	...
	it("replaces history rows when moving to the next page", async () => {
		...
		expect(mocks.listMusicListeningHistory).toHaveBeenCalledWith({
			page: 1,
			page_size: 20,
		});
		...
		expect(mocks.listMusicListeningHistory).toHaveBeenLastCalledWith({
			page: 2,
			page_size: 20,
		});
	...
	it("opens artist and album details from a history row", async () => {
		...
		await flushPromises();
		await wrapper
			.get('[data-testid="history-artist-artist-1"]')
			.trigger("click");
