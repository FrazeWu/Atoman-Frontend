...
export const createFeedCoreState = () => {
	...
	const setSubscriptionGroup = async (
		subId: string,
		groupId: string | null,
	): Promise<boolean> => {
		const authStore = useAuthStore();
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subId}/group`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
	...
	const unsubscribe = async (subscriptionId: string): Promise<boolean> => {
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscriptionId}`,
				{
					method: "DELETE",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const batchUpdateSubscriptions = async (
		...
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated || !ids.length) return false;
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/batch-update`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
			...
			reportError(e, "Failed to batch update subscriptions");
			return false;
		}
	};

	const batchDeleteSubscriptions = async (ids: string[]): Promise<boolean> => {
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated || !ids.length) return false;
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/batch-delete`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			reportError(e, "Failed to batch delete subscriptions");
			return false;
		}
	};
	...
	const setSubscriptionPaused = async (
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${id}/pause`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
			...
			reportError(e, "Failed to update subscription pause state");
			return false;
		}
	};

	const reorderSubscriptionGroups = async (ids: string[]): Promise<boolean> => {
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated || !ids.length) return false;
		try {
			const res = await apiRequestResult(`${api.url}/feed/groups/order`, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${authStore.token}`,
			...
			reportError(e, "Failed to reorder subscription groups");
			return false;
		}
	};
	...
	const reorderSubscriptions = async (
		...
		const authStore = useAuthStore();
		if (!authStore.isAuthenticated || !groupId || !ids.length) return false;
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/groups/${groupId}/subscriptions/order`,
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
			...
			reportError(e, "Failed to reorder subscriptions");
			return false;
		}
	};
	...
	const checkSubscriptionHealth = async (
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscriptionId}/health`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const checkAllSubscriptionsHealth = async (): Promise<boolean> => {
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/health/check-all`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const syncSubscription = async (
		...
		if (
			...
			syncingAllSubscriptions.value ||
			syncingSubscriptionIds.value.has(subscriptionId)
		)
			return null;
		const generation = sessionGeneration;
		...
		try {
			const res = await apiRequestResult(
				`${api.url}/feed/subscriptions/${subscriptionId}/sync`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${authStore.token}` },
				},
	...
	const syncAllSubscriptions =
		async (): Promise<SubscriptionSyncSummary | null> => {
			const authStore = useAuthStore();
			if (
				!authStore.isAuthenticated ||
				syncingAllSubscriptions.value ||
				syncingSubscriptionIds.value.size > 0
			)
				return null;
			const generation = sessionGeneration;
			...
			try {
				const res = await apiRequestResult(
					`${api.url}/feed/subscriptions/sync-all`,
					{
						method: "POST",
						headers: { Authorization: `Bearer ${authStore.token}` },
					},
				...
				subscriptionSyncResults.value = {
					...subscriptionSyncResults.value,
					...Object.fromEntries(
						summary.results.map((result) => [result.subscription_id, result]),
					),
				};
				await fetchSubscriptions();
				return summary;
