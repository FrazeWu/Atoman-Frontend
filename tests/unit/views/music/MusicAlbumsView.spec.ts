...
import AlbumsView from "../../../../src/views/music/AlbumsView.vue";
import { removePendingMusicLyricsAnnotation } from "../../../../src/composables/usePendingMusicLyricsAnnotations";
...
vi.mock("@/views/music/DiscoverView.vue", () => ({
	default: {
		...
		template:
			'<div data-testid="music-explore-view-stub" :data-page-title="pageTitle" :data-content-mode="contentMode">专辑首页</div>',
	},
}));
...
vi.mock("@/components/music/NestedActionDrawer.vue", () => ({
	default: { template: '<div data-testid="nested-action-drawer-stub" />' },
}));
vi.mock("@/components/music/MusicEntityEditorDrawer.vue", () => ({
	default: {
		template: '<div data-testid="music-entity-editor-drawer-stub" />',
	},
}));
vi.mock("@/components/music/MusicCreationFlowDrawer.vue", () => ({
	default: {
		template: '<div data-testid="music-creation-flow-drawer-stub" />',
	},
}));
...
describe("Music AlbumsView.vue (Album Landing)", () => {
	...
	it("renders the album landing content for the music module entry", () => {
		...
		expect(
			wrapper.find('[data-testid="music-explore-view-stub"]').exists(),
		).toBe(true);
		expect(wrapper.text()).toContain("专辑首页");
		expect(
			wrapper
				.find('[data-testid="music-explore-view-stub"]')
				.attributes("data-page-title"),
		).toBe("专辑");
		expect(
			wrapper
				.find('[data-testid="music-explore-view-stub"]')
				.attributes("data-content-mode"),
		).toBe("albums");
	});
	...
	it("shows the authenticated user exact pending rebind count and opens the first task", async () => {
		...
		expect(
			wrapper.get('[data-testid="music-pending-rebind"]').text(),
		).toContain("2");
	...
	it("does not show a pending rebind entry when the authenticated user has no tasks", async () => {
		...
		expect(mocks.listPendingMusicLyricsAnnotations).toHaveBeenCalled();
		expect(wrapper.find('[data-testid="music-pending-rebind"]').exists()).toBe(
			false,
		);
	});
