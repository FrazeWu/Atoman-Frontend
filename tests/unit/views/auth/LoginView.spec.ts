...
describe("LoginView redirect", () => {
  ...
  it("shows configured OAuth providers with the safe return path", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ providers: ["google"] }), { status: 200 }),
    );
    const pinia = createPinia();
    setActivePinia(pinia);
    ...
    expect(
      wrapper.get('[data-test="oauth-provider-google"]').attributes("href"),
  ...
  it("aligns the smaller password reset link to the right side of the footer", () => {
    ...
    const forgotPasswordStyles = loginViewSource.match(
      /\.auth-footer__forgot\s*\{([^}]*)\}/,
    )?.[1];
  ...
  it("renders turnstile only before sending the verification code", () => {
    expect(
      shouldRenderTurnstileForRegisterStep(true, true, "0x4AAAAA", 1),
    ).toBe(true);
    expect(
      shouldRenderTurnstileForRegisterStep(true, true, "0x4AAAAA", 2),
    ).toBe(false);
    expect(
      shouldRenderTurnstileForRegisterStep(true, false, "0x4AAAAA", 1),
    ).toBe(false);
    expect(
      shouldRenderTurnstileForRegisterStep(false, true, "0x4AAAAA", 1),
    ).toBe(false);
  ...
  it("maps fatal turnstile codes to user-facing messages", () => {
    expect(resolveTurnstileErrorMessage(110200)).toBe(
      "当前域名尚未完成验证配置，请稍后再试",
    );
    expect(resolveTurnstileErrorMessage(110100)).toBe(
      "当前无法完成验证，请稍后再试",
    );
    expect(resolveTurnstileErrorMessage(400070)).toBe(
      "当前无法完成验证，请稍后再试",
    );
  ...
  it("validates register usernames against site handle rules", () => {
    ...
    expect(validateRegisterUsername("Alice")).toBe(
      "用户名只能使用小写字母、数字或连字符",
    );
    expect(validateRegisterUsername("a")).toBe(
      "用户名长度需要在 2 到 30 个字符之间",
    );
  ...
  it("uses one turnstile challenge and keeps the completed register button clickable", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (
        ...
        url.endsWith("/auth/check-username")
      ) {
    ...
    expect(
      wrapper.get(".auth-submit-btn").attributes("disabled"),
    ).toBeUndefined();
  ...
  it("prevents duplicate verification sends while the first request is still pending", async () => {
    ...
    const sendVerificationCalls = fetchMock.mock.calls.filter(
      ([input]) => String(input) === "/api/v1/auth/send-verification",
    );
    expect(sendVerificationCalls).toHaveLength(1);
    expect(sendVerificationCalls[0]?.[1]).toEqual(
