...
describe("music v1 starred and playlist adapters", () => {
	...
	it("builds starred and playlist endpoint paths", () => {
		expect(musicV1Endpoints.artistBookmarks()).toBe(
			"/api/v1/music/bookmarks/artists",
		);
		expect(musicV1Endpoints.albumBookmarks()).toBe(
			"/api/v1/music/bookmarks/albums",
		);
		expect(musicV1Endpoints.playlistSongStatus("playlist-1")).toBe(
			"/api/v1/music/playlists/playlist-1/songs/status",
		);
		expect(musicV1Endpoints.playlistBookmarks()).toBe(
			"/api/v1/music/bookmarks/playlists",
		);
		expect(musicV1Endpoints.playlistBookmark("playlist-1")).toBe(
			"/api/v1/music/bookmarks/playlists/playlist-1",
		);
		expect(musicV1Endpoints.playlists()).toBe("/api/v1/music/playlists");
		expect(musicV1Endpoints.playlistSongs("playlist-1")).toBe(
			"/api/v1/music/playlists/playlist-1/songs",
		);
		expect(musicV1Endpoints.artistMerge("artist-target")).toBe(
			"/api/v1/admin/artists/artist-target/merge",
		);
		expect(musicV1Endpoints.albumRevisions("album-1")).toBe(
			"/api/v1/albums/album-1/revisions",
		);
		expect(musicV1Endpoints.albumDiscussions("album-1")).toBe(
			"/api/v1/discussions/music_album/album-1/comments",
		);
	});
	...
	it("queries and mutates playlist song membership", async () => {
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				if (url.includes("/status")) {
					return new Response(
						JSON.stringify({ data: { song_ids: ["song-1"] } }),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					);
				}
				if (
					url === "/api/v1/music/playlists/playlist-1/songs" &&
					init?.method === "POST"
				) {
					return new Response(
						JSON.stringify({
							data: { playlist_id: "playlist-1", song_id: "song-1" },
						}),
						{ status: 201, headers: { "Content-Type": "application/json" } },
					);
				}
				if (
					url === "/api/v1/music/playlists/playlist-1/songs/song-1" &&
					init?.method === "DELETE"
				) {
		...
		expect(
			await getMusicPlaylistSongStatus("playlist-1", ["song-1", "song-2"]),
		).toEqual(["song-1"]);
	...
	it("lists bookmarks and playlists through existing music endpoints", async () => {
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL) => {
				...
				if (url === "/api/v1/music/playlists") {
					return new Response(
						JSON.stringify({
							data: [{ id: "playlist-1", name: "夜航歌单", song_count: 2 }],
							meta: { page: 1, page_size: 20, total: 1, has_more: false },
						}),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					);
				...
				if (url === "/api/v1/music/artists/artist-1") {
					return new Response(
						JSON.stringify({
							data: { id: "artist-1", name: "FKA twigs", entry_status: "open" },
						}),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					);
				}
				if (url === "/api/v1/music/albums/album-1") {
					return new Response(
						JSON.stringify({
							data: { id: "album-1", title: "MAGDALENE", entry_status: "open" },
						}),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					);
				}
		...
		expect(starred.map((item) => item.kind)).toEqual([
			"artist",
			"album",
			"playlist",
		]);
	...
	it("creates and deletes playlist bookmarks through the playlist bookmark endpoint", async () => {
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
				if (
					String(input) !== "/api/v1/music/bookmarks/playlists" &&
					String(input) !== "/api/v1/music/bookmarks/playlists/playlist-1"
				) {
	...
	it("renames and deletes playlists through the music namespace", async () => {
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
				const url = String(input);
				if (
					url === "/api/v1/music/playlists/playlist-1" &&
					init?.method === "PATCH"
				) {
					return new Response(
						JSON.stringify({
		...
		const updated = await updateMusicPlaylist("playlist-1", {
			name: "晚间歌单",
			description: "夜里听",
		});
		...
		expect(fetch).toHaveBeenNthCalledWith(
			1,
			"/api/v1/music/playlists/playlist-1",
			{
				method: "PATCH",
				credentials: "include",
				headers: {
	...
	it("reorders playlist songs through the music namespace", async () => {
		...
		const result = await reorderMusicPlaylistSongs("playlist-1", [
			"song-2",
			"song-1",
		]);

		expect(fetch).toHaveBeenCalledWith(
			"/api/v1/music/playlists/playlist-1/songs/order",
			{
				method: "PUT",
				credentials: "include",
				headers: {
		...
		vi.stubGlobal(
			...
			vi.fn(
				async () =>
					new Response(
						JSON.stringify({
							data: { message: "Artists merged successfully" },
						}),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					),
			),
	...
	it("merges albums through the music namespace", async () => {
		vi.stubGlobal(
			...
			vi.fn(
				async () =>
					new Response(
						JSON.stringify({
							data: { merged: true, redirect_to: "album-target" },
						}),
						{ status: 200, headers: { "Content-Type": "application/json" } },
					),
			),
		...
		expect(fetch).toHaveBeenCalledWith(
			"/api/v1/music/albums/album-target/merge",
			{
				method: "POST",
				credentials: "include",
				headers: {
				...
				body: JSON.stringify({
					...
					confirmed: true,
					song_matches: [
						{ source_song_id: "song-source", target_song_id: "song-target" },
					],
				}),
			},
		);
	...
	it("gets playlist details with songs", async () => {
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL) => {
				...
				if (
					url === "/api/v1/music/playlists/playlist-1/songs?page=1&page_size=20"
				) {
					return new Response(
						JSON.stringify({
							data: [
		...
		expect(fetch).toHaveBeenCalledWith(
			"/api/v1/music/playlists/playlist-1/songs?page=1&page_size=20",
			{
				credentials: "include",
