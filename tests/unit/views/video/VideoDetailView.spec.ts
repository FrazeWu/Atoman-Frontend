...
describe("VideoDetailView shared interactions", () => {
	...
	it("在当前视频信息之后展示推荐视频", async () => {
		...
		expect(wrapper.get('.vd-main').classes()).toContain("vd-main--compact");
		expect(wrapper.getComponent(PVideoPlayerShellStub).classes()).not.toContain("vd-player-shell--compact");
	...
	it("opens comments in the shared right sheet and keeps the rating control in the interaction row", async () => {
		...
		expect(comments.props("partialWidth")).toBe("var(--a-recommendation-width)");
