...
const debateRow = {
  id: "debate-1",
  user_id: "user-1",
  user: { username: "fafa", email: "fafa@example.com" },
  ...
  tags: [],
  view_count: 2,
  current_revision_id: "revision-debate-1",
  references: [],
  conclusion_type: "yes",
  ...
  return shallowMount(DebateHomeView, {
    global: {
      stubs: {
        ...
        PContentCard: {
          template:
            '<article><slot /><slot name="meta" /><slot name="title" /><slot name="summary" /><slot name="actions" /></article>',
        },
        PInteractionCard: {
          template: '<article class="debate-card-stub"><slot /></article>',
        },
        PSelect: {
          props: ["options"],
          template: `
        ...
        PInput: {
          ...
          template:
            '<label>{{ label }}<input :data-label="label" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /></label>',
        },
        PReferenceField: {
          props: ["modelValue", "label"],
          emits: ["update:modelValue"],
          template:
            '<label>{{ label }}<textarea :data-label="label" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /></label>',
        },
        PEmpty: true,
...
describe("DebateHomeView node wording", () => {
  beforeEach(() => {
    ...
    vi.mocked(fetch).mockImplementation(async (_input, init) => {
      if (init?.method === "POST") {
        return new Response(JSON.stringify({ data: createResponse }), {
          status: 201,
        });
      }
      return new Response(
  ...
  it("treats every item as a debate node and uses the unified conclusion stamp", async () => {
    ...
    expect(
      wrapper.findAll('[data-test="status-filter"] option').map((option) => ({
        label: option.text(),
  ...
  it("创建时只提交标题、正文、标签并用空描述", async () => {
    ...
    await wrapper
      .get('[data-label="标题"]')
      .setValue("每周运动会不会降低心血管疾病风险？");
    await wrapper
      .get('[data-label="正文"]')
      .setValue("汇总随机试验和队列研究。");
