...
describe("MusicLayout.vue", () => {
	...
	it("mounts one semantic music sheet stack", () => {
		const wrapper = mountLayout();
		expect(
			wrapper.findAll('[data-testid="music-sheet-stack-stub"]'),
		).toHaveLength(1);
	});
	...
	it("preserves a detail route shell after returning from a stacked detail route", async () => {
		...
		await nextTick();
		expect(wrapper.find('[data-testid="artist-detail-route"]').exists()).toBe(
			true,
		);

		await router.push("/album/album-1");
		await nextTick();
		expect(wrapper.find('[data-testid="album-detail-route"]').exists()).toBe(
			true,
		);
