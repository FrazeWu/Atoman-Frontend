...
): MobilePrimaryTab[] => entries.map(([key, label, path, icon]) => ({
  key,
  label,
  module,
...
const MORE_ITEMS: MobileMoreItem[] = [
  ...
  { module: 'forum', label: '论坛', href: moduleUrl('forum'), icon: MessageSquare },
  { module: 'debate', label: '辩题', href: moduleUrl('debate'), icon: MessageSquare },
  { module: 'timeline', label: '时间线', href: moduleUrl('timeline'), icon: Radio },
  { module: 'podcast', label: '播客', href: moduleUrl('podcast'), icon: ListMusic },
  { module: 'video', label: '视频', href: moduleUrl('video'), icon: Compass },
]

export const getMobilePrimaryTabs = (module?: ModuleRoomKey): MobilePrimaryTab[] => (
