...
test.describe("Unified Studio", () => {
	test.beforeEach(async ({ page }) => {
		...
		await page.route("**/api/v1/**", async (route) => {
						...
						? [
								{
									...
									status: "published",
									visibility: "public",
									collections: [{ id: "collection-default", name: "全部文章" }],
									view_count: 12,
									metrics: {
										view: 12,
										comment: 3,
			...
			if (path === "/api/v1/blog/posts/post-1/unpublish" && method === "POST")
				return fulfill({ id: "post-1", status: "draft" });
			if (
				path === "/api/v1/studio/blog/contents/post-1/share" &&
				method === "POST"
			)
				return fulfill({ data: { path: "/posts/post/post-1" } });
			...
			if (path === "/api/v1/blog/drafts" && method === "GET")
				return fulfill({ data: null });
			...
			if (path === "/api/v1/blog/drafts" && method === "DELETE")
				return fulfill({ data: { ok: true } });
			if (path === "/api/v1/blog/posts" && method === "POST")
				return fulfill({ data: { id: "draft-1" } });

			return fulfill({ data: [] });
		});
	});

	test("manages all creator modules under one channel and keeps collection context", async ({
		page,
	}, testInfo) => {
		...
		for (const entry of [
			{
				path: "/posts",
				testId: "module-create-blog",
				label: "写博客",
				href: "/studio/blog/new",
			},
			{
				path: "/podcasts",
				testId: "module-create-podcast",
				label: "上传播客",
				href: "/studio/podcast/new",
			},
			{
				path: "/videos",
				testId: "module-create-video",
				label: "上传视频",
				href: "/studio/video/new",
			},
		]) {
			await navigateInApp(entry.path);
			const createLink = page.getByTestId(entry.testId);
			await expect(createLink).toContainText(entry.label);
			await expect(createLink).toHaveAttribute("href", entry.href);
		}
		...
		await expect(
			page.locator('[data-testid="studio-channel-selector"] select'),
		).toHaveValue("channel-1");
		await expect(
		...
		await expect(
			page
				.getByTestId("studio-collection-manager")
		...
		await collectionFilter
			.getByRole("option", { name: "研究", exact: true })
			.click();
