...
import {
  loadReadingListFeedItemIds,
  loadStarredFeedItemIds,
} from "@/api/feedMembership";
...
export function createFeedMembershipState() {
  ...
  const fetchBookmarkedPostIds = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const bookmarks = (data.data || []) as Array<{
          content_id?: unknown;
          post_id?: unknown;
        }>;
        const ids = bookmarks
          .map((bookmark) => {
            ...
            return typeof bookmark.content_id === "string"
              ? bookmark.content_id
              : "";
          })
          .filter(Boolean);
    ...
    try {
      if (bookmarkedPostIds.value.has(postId)) {
        ...
        const bookmark = (data.data || []).find(
          (item: { content_id?: unknown; post_id?: unknown }) =>
            item.post_id === postId || item.content_id === postId,
        ) as { id?: string } | undefined;
        if (!bookmark?.id) return null;
        const deleteRes = await apiRequestResult(
          `${api.url}/blog/bookmarks/${bookmark.id}`,
          {
            method: "DELETE",
            headers: { Authorization: `Bearer ${authStore.token}` },
          },
  ...
  const moveStarToGroup = async (
    ...
    try {
      const res = await apiRequestResult(
        `${api.url}/feed/stars/${feedItemId}/group`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const syncStarredPageIds = (previousIds: string[], nextIds: string[]) => {
    ...
    starredItemIds.value = mergePendingMembership(next, starToggleStates);
  };
  ...
  const clearMembershipState = () => {
    ...
    readingListMembershipRevision += 1;
    for (const state of [
      ...starToggleStates.values(),
      ...readingListToggleStates.values(),
    ]) {
      state.waiters.splice(0).forEach((resolve) => resolve(null));
    }
