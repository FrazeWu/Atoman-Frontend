...
vi.mock("vue-router", () => ({
    useRoute: () => ({
        query: routeQuery,
        fullPath: `/inbox?tab=${routeQuery.tab || ""}`,
    }),
...
describe("InboxPage notifications", () => {
    ...
    it("opens a source health alert at subscription management", async () => {
        routeQuery = { tab: "system" };
        const store = useNotificationStore();
        store.notifications = [
            {
                id: "feed-health-1",
                recipient_id: "u1",
                type: "feed_source_health",
                category: "system",
                reason: "",
                source_type: "feed_source_health",
                source_id: "source-1",
                actor_count: 1,
                meta: {
                    title: "订阅源需要处理",
                    body: "Example Feed 已连续抓取失败 3 次，系统会自动重试。",
        ...
        expect(routerPush).toHaveBeenCalledWith(
            "/feed?manage_subscriptions=1&manage_tab=sources",
        );
    });
    ...
    it("shows notification source label in list and detail panes", async () => {
        ...
        expect(wrapper.find(".sidebar-item-source").text()).toBe(
            "测试通知来源：点赞",
        );
        await wrapper.find(".sidebar-item").trigger("click");
        await wrapper.vm.$nextTick();
        expect(wrapper.find(".detail-source").text()).toBe(
            "测试通知来源：点赞",
        );
    });
    ...
    it("disables dm composer for blocked conversation", () => {
        ...
        dm.reconcile({
            mailboxes: [
                { type: "user", id: "me", display_name: "Me", unread_count: 0 },
            ],
    ...
    it("restores a conversation from the v2 conversation query", async () => {
        ...
        await vi.waitFor(() =>
            expect(open).toHaveBeenCalledWith("conversation-1"),
        );
    });
});
