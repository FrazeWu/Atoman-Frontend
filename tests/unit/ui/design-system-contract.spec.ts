...
describe("modern flat UI design-system contract", () => {
  ...
  it("keeps shared cards quiet until interaction", () => {
    ...
    expect(source).toContain("border: 1px solid var(--a-color-border-soft);");
    expect(source).toContain("border-radius: var(--a-radius-card);");
    expect(source).not.toContain("box-shadow: var(--a-shadow-sm);");
    expect(source).not.toMatch(
      /dashed|border-left|text-decoration:\s*underline/,
    );
  });
  ...
  it("uses neutral shared field naming and standard form styling", () => {
    ...
    expect(textarea).not.toMatch(
      /a-font-mono|p-field-dot|Ruled lines|background-image/,
    );
