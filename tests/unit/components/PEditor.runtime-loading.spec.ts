...
const editorShellSource = readFileSync(
  path.resolve(process.cwd(), 'src/components/shared/PEditor.vue'),
  'utf8',
)

describe('PEditor runtime loading', () => {
  it('keeps the heavy editor runtime behind an async boundary', () => {
    ...
    expect(editorShellSource).toContain("import('./PEditorRuntime.vue')")
  })
})