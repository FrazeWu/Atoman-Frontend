...
describe("PostEditorView", () => {
		...
		const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
			...
			if (url.includes("/users/me/default-channels")) {
				return makeJsonResponse({
					data: { blog: null, podcast: null, video: null },
				});
			}
	...
	it("新建文章补默认频道时，应保留已注册的编辑器路由前缀", async () => {
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL) => {
				...
				if (url.includes("/blog/channels?")) {
					return makeJsonResponse({
						data: [{ id: "channel-1" }, { id: "channel-2" }],
					});
				}
				if (url.includes("/blog/channels/channel-2/collections")) {
					return makeJsonResponse({ data: [] });
	...
	it("新建文章应在合法频道下恢复 query.collection", async () => {
		...
		await router.push(
			"/posts/post/new?channel=channel-2&collection=collection-2",
		);
		await router.isReady();
	...
	it("新建文章不得恢复不属于当前频道的非法 query.collection", async () => {
		...
		await router.push(
			"/posts/post/new?channel=channel-2&collection=collection-x",
		);
		await router.isReady();
	...
	it("选择恢复草稿后，调用 replaceDocument，把草稿内容写回协作文档", async () => {
		...
		expect(editorControl.replaceDocument).toHaveBeenCalledWith(
			"# 本地标题\n本地正文",
		);
	...
	it("编辑态在协作未 ready 前，仍然会保存本地草稿", async () => {
		...
		const savedDraft = JSON.parse(
			localStorage.getItem("blog_editor_blog:post:post-1") || "null",
		);
	...
	it("协作文档为空时应自动把草稿写回共享文档", async () => {
		...
		const router = createRouter({
			history: createMemoryHistory(),
			routes: [{ path: "/post/:id/edit", component: PostEditorView }],
		});
	...
	it("编辑已有文章保存时，会提交当前频道和合集", async () => {
		...
		const fetchMock = vi.fn(
			async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				if (url.includes("/blog/channels?"))
					return makeJsonResponse({ data: [] });
				if (
					url.includes("/blog/posts/post-1") &&
		...
		const putCall = fetchMock.mock.calls.find(
			([input, init]) =>
				String(input).includes("/blog/posts/post-1") && init?.method === "PUT",
		);
	...
	it("应当根据 URL 查询参数初始化合集和频道", async () => {
		...
		await router.push(
			"/posts/post/new?channel=channel-2&collection=collection-3",
		);
		await router.isReady();
	...
	it("使用 Studio 当前频道和合集预选，草稿保存后返回内容管理", async () => {
		...
		const postCall = fetchMock.mock.calls.find(
			([input, init]) =>
				String(input).endsWith("/blog/posts") && init?.method === "POST",
		);
	...
	it("新文章应用 Studio 创作默认值", async () => {
		...
		studio.settings.blog = {
			channel_id: "channel-1",
			module: "blog",
			default_collection_id: "collection-2",
		...
		vi.stubGlobal(
			"fetch",
			vi.fn(async (input: RequestInfo | URL) => {
				if (String(input).includes("/blog/drafts?context_key="))
					return makeJsonResponse({ data: null });
			...
			vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				if (url.endsWith("/blog/posts") && init?.method === "POST") {
					return new Response(
						JSON.stringify({
							error: {
								code: "reference.invalid_syntax",
								message: "Reference syntax is invalid",
							},
						}),
						{ status: 400 },
					);
