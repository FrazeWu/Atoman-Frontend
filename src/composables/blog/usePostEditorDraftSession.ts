...
export function usePostEditorDraftSession({
  ...
  const editorSessionState = ref<EditorSessionState>(isEdit.value ? 'awaiting-collab' : 'local-edit')
  const collabStartupFallbackTriggered = ref(false)

  let serverSyncTimer: ReturnType<typeof setTimeout> | null = null
  let collabStartupTimer: ReturnType<typeof setTimeout> | null = null

  const hasMeaningfulDraft = (payload: EditorDraftPayload) => Boolean(
    payload.title.trim()
  ...
  const draftStatus = computed<DraftStatus>(() => {
    ...
    if (deferredDraftCandidate.value) return { tone: 'warn', text: '检测到可恢复草稿' }
  ...
  const draftRecoveryPreview = computed(() => draftPreview(pendingDraftCandidate.value))
  const deferredDraftSummary = computed(() => draftPreview(deferredDraftCandidate.value))
  ...
  const localDraftStatusText = computed(() => {
    if (lastSavedAt.value) return `最近保存于 ${formatSavedTime(lastSavedAt.value)}`
    if (autoSaveState.value === 'saving') return '正在保存中…'
    if (autoSaveState.value === 'error') return '本地草稿保存失败'
    if (hasMeaningfulDraft(draftPayload.value)) return '已有编辑内容，等待下一次自动保存'
    return '暂无本地草稿'
  })
  const cloudDraftStatusText = computed(() => {
    ...
    if (serverDraftSavedAt.value) return `最近同步于 ${formatSavedTime(serverDraftSavedAt.value)}`
    if (hasMeaningfulDraft(draftPayload.value)) return '尚未生成云端草稿'
    return '暂无云端草稿'
  })
  const leaveConfirmText = computed(() => {
    if (saving.value === 'published') return '文章正在发布中，离开后可能无法确认本次发布结果。'
    if (saving.value === 'draft') return '文章正在保存草稿，离开后本次保存可能无法完成。'
  ...
  const getLatestDraftCandidate = async () => {
    ...
    if (localDraft && hasMeaningfulDraft(localDraft.payload)) {
      ...
      candidates.push({ source: 'local', payload: localDraft.payload, savedAt: localDraft.saved_at })
    }
    if (serverDraft) {
  ...
  const scheduleServerDraftSync = () => {
    ...
    serverSyncTimer = setTimeout(() => { void syncServerDraft() }, 1800)
  }
  ...
  const evaluateDraftRecovery = async () => {
    ...
    if (isEdit.value && latestCandidate.savedAt <= loadedPostUpdatedAt.value) return
  ...
  const openDraftManager = () => { draftManagerVisible.value = true }
  const closeDraftManager = () => { draftManagerVisible.value = false }
  const restorePendingDraft = async () => {
    ...
    if (isCollabConflict.value) {
      ...
      replaceDocument(`# ${candidate.payload.title}\n${candidate.payload.content}`)
      deferredDraftCandidate.value = null
  ...
  const handleCollabReady = async (markdown: string) => {
    ...
    if (!hasMeaningfulMarkdown(markdown) && hasMeaningfulDraft(latestCandidate.payload)) {
  ...
  const allowNextRouteLeave = () => { allowRouteLeaveOnce.value = true }
  ...
  const startDraftSession = async () => {
    ...
    if (isEdit.value) {
      ...
      clearCollabStartupTimer()
      collabStartupTimer = setTimeout(() => { void fallbackCollabRecovery() }, 1500)
      return
    }
    await evaluateDraftRecovery()
  }
