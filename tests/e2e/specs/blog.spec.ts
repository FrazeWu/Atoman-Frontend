...
const testChannel = { id: 'channel-1', name: '测试频道', slug: 'test', description: '', cover_url: '' }

async function mockBlogAuthor(page: Page) {
  await mockAuthenticatedSession(page, {
    uuid: 'user-1', username: 'admin', email: 'admin@example.com', role: 'admin',
    onboarding_completed_at: '2026-07-15T09:00:00Z',
  })
  await page.route('**/api/v1/studio/state', route => route.fulfill({
    status: 200,
    json: { data: { current_channel: testChannel, channels: [testChannel] } },
  }))
  await page.route('**/api/v1/studio/blog/collections', route => route.fulfill({ status: 200, json: { data: [] } }))
  await page.route('**/api/v1/studio/blog/settings', route => route.fulfill({
    status: 200,
    json: { data: { channel_id: testChannel.id, module: 'blog', default_visibility: 'public', default_publish_status: 'draft' } },
  }))
}
...
test.describe('Blog', () => {
  ...
  test('visit blog settings page', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: '账号设置', exact: true })).toBeVisible()
  })
  ...
  test('editor uses the workbench compose workflow', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('button', { name: '发布' })).toBeVisible()
  })
})
