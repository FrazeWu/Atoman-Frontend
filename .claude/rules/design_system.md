...
- 双栏布局使用共享 token：主内容区占剩余空间，推荐区使用 `var(--a-recommendation-width)`（标准值 `20rem`），列间距使用 `var(--a-content-column-gap)`（标准值 `2rem`）。目标视觉比例约为主内容 `70%`、推荐区 `30%`。推荐区本身保持 `20rem`，不因 Sheet 宽度规则改变。
...
2. `partial`：统一使用 `var(--a-comment-sheet-width)`（标准值 `42rem`），覆盖右侧附加内容区。所有 partial Sheet 都必须使用这个共享 token，组件不得自行写死宽度或回退到推荐栏宽度。

完整编辑、详情和创建流程使用 `full`；评论、相关推荐和附加信息使用统一 `42rem` 的 `partial`。Sheet 通过 Teleport 渲染时，边界仍以外层内容区为准，不能受内层最大宽度影响。
