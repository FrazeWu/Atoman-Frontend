...
import StudioDashboardView from "../../../../src/views/studio/StudioDashboardView.vue";
...
describe("StudioDashboardView", () => {
  it("renders channel summary, prioritized actions, recent content, and module cards", async () => {
    ...
    const wrapper = mount(StudioDashboardView, {
      global: { plugins: [pinia], stubs: { RouterLink } },
    });
    await flushPromises();

    expect(
      wrapper.find('[data-testid="dashboard-subscriber-count"]').text(),
    ).toContain("23");
    ...
    expect(wrapper.find(".studio-dashboard__actions").text()).toContain(
      "2 个视频处理失败",
    );
    expect(wrapper.find(".studio-dashboard__recent").text()).toContain(
      "blog 最近内容",
    );
    expect(
      wrapper
        .findAll('[data-testid="studio-dashboard-section"]')
        .map((item) => item.attributes("data-module")),
    ).toEqual(["blog", "podcast", "video"]);
    expect(
      wrapper.find('[data-module="blog"] [data-metric="view"]').text(),
    ).toContain("70");
    expect(
      wrapper.find('[data-module="podcast"] [data-metric="play"]').text(),
    ).toContain("50");
    expect(
      wrapper.find('[data-module="video"] [data-metric="play"]').text(),
    ).toContain("30");
    expect(wrapper.find('a[href="/studio/manage/channel"]').exists()).toBe(
      true,
    );
  ...
  it("routes stale drafts and scheduled content to actionable views", async () => {
    ...
    const blog = section("blog", 2);
    blog.issues = [
      { code: "stale_draft", count: 1 },
      { code: "scheduled", count: 2 },
    ];
    store.dashboard = {
      channel_subscriber_count: 0,
      sections: [blog, section("podcast", 0), section("video", 0)],
    };
    ...
    expect(
      wrapper.find('a[href="/studio/blog/content?issue=stale_draft"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('a[href="/studio/blog/content?status=scheduled"]').exists(),
    ).toBe(true);
  });
  ...
  it("keeps successful module cards visible when one section fails", async () => {
    ...
    expect(wrapper.find('[data-module="podcast"]').text()).toContain(
      "播客加载失败",
    );
    expect(
      wrapper.find('[data-module="blog"] [data-metric="view"]').text(),
    ...
    await wrapper
      .find('[data-module="podcast"] [data-testid="retry-dashboard-section"]')
      .trigger("click");
    await flushPromises();
