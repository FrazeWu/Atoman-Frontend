...
describe("podcast routing prefix", () => {
	...
	it("re-fetches and clears a previous show when channelSlug changes in the reused route component", async () => {
		...
		const fetchMock = vi.fn((url: string) => {
			if (url.endsWith("/podcast/shows/show-a/episodes"))
				return Promise.resolve(makeJsonResponse(show("show-a", "节目 A")));
		...
		const router = createRouter({
			history: createMemoryHistory(),
			routes: [
				{ path: "/podcasts/show/:channelSlug", component: PodcastShowView },
			],
		});
		...
		expect(
			fetchMock.mock.calls.some(([url]) =>
				String(url).includes("/podcast/shows/show-b/episodes"),
			),
		).toBe(true);
	...
	it("re-fetches and clears a previous episode when id changes in the reused route component", async () => {
		const secondResponse = deferredResponse();
		const fetchMock = vi.fn((url: string) => {
			if (url.endsWith("/podcast/episodes/episode-a"))
				return Promise.resolve(
					makeJsonResponse(episode("episode-a", "单集 A")),
				);
			if (url.endsWith("/podcast/episodes/episode-b"))
				return secondResponse.promise;
			return Promise.resolve(
				makeJsonResponse({ data: [], meta: { total: 0 } }),
			);
		});
		vi.stubGlobal("fetch", fetchMock);
		const router = createRouter({
		...
		expect(
			fetchMock.mock.calls.some(([url]) =>
				String(url).includes("/podcast/episodes/episode-b"),
			),
		).toBe(true);
