...
describe("PostEditorView layering", () => {
  it("uses blog building blocks instead of duplicating sidebar and topbar structure", () => {
    const source = readFileSync(
      path.resolve(process.cwd(), "src/views/blog/PostEditorView.vue"),
      "utf8",
    );
...
describe("Blog sheet layering", () => {
  it.each(["BlogCollectionSheet.vue", "BlogPostSheet.vue"])(
    "marks only the top %s as modal",
    (file) => {
      const source = readFileSync(
        path.resolve(process.cwd(), `src/components/blog/${file}`),
        "utf8",
      );
