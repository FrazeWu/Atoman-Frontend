...
import {
	createSheetStack,
	type BaseSheetLayer,
} from "../../../src/composables/useSheetStack";
...
describe("createSheetStack", () => {
	...
	it("pushes, replaces, pops, and returns to a retained layer", () => {
		...
		expect(stack.layers.value.map((item) => item.key)).toEqual([
			"artist:1",
			"history:3",
		]);
	...
	it("returns to an existing lower layer and keeps removed layers mounted through the leave transition", async () => {
		vi.useFakeTimers();
		const stack = createSheetStack<TestLayer>();
		stack.push(layer("artist", "1"));
		stack.push(layer("album", "2"));
		...
		expect(stack.layers.value.map((item) => item.key)).toEqual(["artist:1"]);
		expect(stack.renderLayers.value.map((item) => item.key)).toEqual([
			"artist:1",
			"album:2",
		]);
