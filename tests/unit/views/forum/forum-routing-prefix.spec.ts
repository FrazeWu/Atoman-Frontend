    ...
    global: {
      ...
      stubs: {
        ...
        PPageHeader: {
          emits: ["back"],
          template:
            '<header><button class="back" @click="$emit(\'back\')">back</button><slot /><slot name="action" /></header>',
        },
        PEmpty: { template: "<div />" },
        PInput: {
          props: ["modelValue"],
        ...
        PContentCard: {
          emits: ["click"],
          template:
            '<article class="p-entry" @click="$emit(\'click\')"><slot name="meta" /><slot name="title" /><slot name="actions" /></article>',
        },
...
describe("forum 路由前缀", () => {
  ...
  it("论坛首页创建、点击话题和键盘 Enter 都跳转到 /forum 前缀下", async () => {
    const { wrapper, pushSpy } = await mountWithRouter(
      ForumHomeView,
      "/forum",
      () => {
    ...
    await wrapper
      .findAll("button")
      .find((button) => button.text().includes("发新话题"))!
      .trigger("click");
  ...
  it("论坛搜索页返回、结果点击和搜索 replace 都保留 /forum 前缀", async () => {
    ...
    await wrapper
      .findAll("button")
      .find((button) => button.text().includes("返回论坛"))!
      .trigger("click");
  ...
  it("发布话题成功后跳转到 /forum/topic/:id", async () => {
    const { wrapper, pushSpy } = await mountWithRouter(
      ...
      () => {
        ...
        vi.spyOn(forumStore, "createTopic").mockResolvedValue(
          makeTopic("topic-new"),
        );
    ...
    await wrapper
      .findAll("button")
      .find((button) => button.text().includes("发布话题"))!
      .trigger("click");
    await flushPromises();
    ...
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          data: {
            items: [],
            target: { liked: false, like_count: 0, comment_count: 0 },
          },
        }),
        { status: 200 },
      ),
  ...
  it("话题作者旁显示论坛等级", async () => {
    ...
    expect(wrapper.get('[data-testid="forum-topic-trust-level"]').text()).toBe(
      "等级 2",
    );
  });
});
