...
const routeState = vi.hoisted(() => ({ route: { params: { id: 'channel-1' } } }))
...
const channel = (id: string, name: string) => ({ id, user_id: 'other-user', name, slug: id })
const collection = (id: string, name: string) => ({ id, name, description: '', channel_id: id.replace('collection-', 'channel-') })
...
describe('ChannelView', () => {
  ...
  it('路由从 A 切到 B 时重新请求并立即清空 A 的频道派生状态', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/feed/subscribe/channel/channel-1/status')) return response({ subscribed: true })
      if (url.includes('/blog/bookmarks')) return response({ data: [] })
      if (url.includes('/feed/reading-list')) return response({ items: [] })
      if (url.includes('/blog/channels/slug/channel-2') && !url.includes('/collections')) return channelB.promise
      if (url.includes('/blog/channels/slug/channel-2/collections')) return response({ data: [collection('collection-2', '合集 B')] })
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-2')) return response({ data: [post('post-2', 'channel-2', '文章 B')] })
      if (url.includes('/feed/subscribe/channel/channel-2/status')) return response({ subscribed: false })
  ...
  it('A 的迟到频道响应不能覆盖已加载的 B', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      const url = String(input)
      if (url.includes('/blog/channels/slug/channel-1') && !url.includes('/collections')) return channelA.promise
      if (url.includes('/blog/channels/slug/channel-2') && !url.includes('/collections')) return channelB.promise
      if (url.includes('/blog/channels/slug/channel-2/collections')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-2')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/feed/subscribe/channel/channel-2/status')) return Promise.resolve(response({ subscribed: false }))
      if (url.includes('/blog/bookmarks')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/feed/reading-list')) return Promise.resolve(response({ items: [] }))
      throw new Error(`unexpected request: ${url}`)
    })
  ...
  it('A 的迟到合集和文章响应不能覆盖已加载的 B', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      const url = String(input)
      if (url.includes('/blog/channels/slug/channel-1') && !url.includes('/collections')) {
        return Promise.resolve(response({ data: channel('channel-1', '频道 A') }))
      }
      if (url.includes('/blog/channels/slug/channel-1/collections')) return collectionsA.promise
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-1')) return postsA.promise
      if (url.includes('/feed/subscribe/channel/channel-1/status')) return Promise.resolve(response({ subscribed: false }))
      if (url.includes('/blog/channels/slug/channel-2') && !url.includes('/collections')) {
        return Promise.resolve(response({ data: channel('channel-2', '频道 B') }))
      }
      if (url.includes('/blog/channels/slug/channel-2/collections')) {
        return Promise.resolve(response({ data: [collection('collection-2', '合集 B')] }))
      }
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-2')) {
        return Promise.resolve(response({ data: [post('post-2', 'channel-2', '文章 B')] }))
      }
      if (url.includes('/feed/subscribe/channel/channel-2/status')) return Promise.resolve(response({ subscribed: false }))
      if (url.includes('/blog/bookmarks')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/feed/reading-list')) return Promise.resolve(response({ items: [] }))
    ...
    collectionsA.resolve(response({ data: [collection('collection-1', '合集 A')] }))
  ...
  it('从已选中的 A 合集切换到 B 时同步清空 activeCollectionId', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/blog/channels/slug/channel-2/collections')) return response({ data: [] })
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-2')) return response({ data: [] })
      if (url.includes('/feed/subscribe/channel/channel-2/status')) return response({ subscribed: false })
      if (url.includes('/blog/bookmarks')) return response({ data: [] })
      if (url.includes('/feed/reading-list')) return response({ items: [] })
      throw new Error(`unexpected request: ${url}`)
  ...
  it('A 的迟到第二页文章响应不会污染已加载的 B', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (url.includes('/blog/channels/slug/channel-1/collections')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-1') && url.includes('page=1')) {
        return Promise.resolve(response({ data: [post('post-1', 'channel-1', '文章 A 第一页')], meta: { has_more: true } }))
      }
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-1') && url.includes('page=2')) return postsASecondPage.promise
      if (url.includes('/feed/subscribe/channel/channel-1/status')) return Promise.resolve(response({ subscribed: false }))
      if (url.includes('/blog/channels/slug/channel-2') && !url.includes('/collections')) {
        return Promise.resolve(response({ data: channel('channel-2', '频道 B') }))
      }
    ...
    routeState.route.params.id = 'channel-2'
    await flushPromises()
    expect(wrapper.vm.$.setupState.channelPosts.map((item: { title: string }) => item.title)).toEqual(['文章 B'])

    postsASecondPage.resolve(response({ data: [post('post-3', 'channel-1', '文章 A 第二页')], meta: { has_more: false } }))
    await flushPromises()
  ...
  it.each([
    ...
    const jsonRejectingResponse = () => ({ ok: true, json: vi.fn().mockRejectedValue(new Error(`${failure} json failed`)) }) as unknown as Response
    const unhandledRejections: PromiseRejectionEvent[] = []
    const onUnhandledRejection = (event: PromiseRejectionEvent) => unhandledRejections.push(event)
    window.addEventListener('unhandledrejection', onUnhandledRejection)

    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      const url = String(input)
      if (url.includes('/blog/channels/slug/channel-1') && !url.includes('/collections')) {
        return failure === 'channel' ? jsonRejectingResponse() : response({ data: channel('channel-1', '频道 A') })
      }
      if (url.includes('/feed/subscribe/channel/channel-1/status')) {
        return failure === 'subscription' ? Promise.reject(new Error('subscription request failed')) : response({ subscribed: true })
      }
      if (url.includes('/blog/channels/slug/channel-1/collections')) {
        return failure === 'collections' ? jsonRejectingResponse() : response({ data: [collection('collection-1', '合集 A')] })
      }
      if (url.includes('/blog/posts?') && url.includes('channel_id=channel-1')) {
        return failure === 'posts' ? jsonRejectingResponse() : response({ data: [post('post-1', 'channel-1', '文章 A')] })
      }
      if (url.includes('/blog/bookmarks')) return response({ data: [] })
      if (url.includes('/feed/reading-list')) return response({ items: [] })
      throw new Error(`unexpected request: ${url}`)
  ...
  it('频道加载链请求失败时不会向调用方传播拒绝', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/feed/subscribe/channel/channel-1/status')) return response({ subscribed: false })
      if (url.includes('/blog/channels/slug/channel-1/collections')) return response({ data: [] })
    ...
    await expect(state.fetchCollections(loadedChannel, 'channel-1', true, 1)).resolves.toBeUndefined()
  ...
  it('A 的迟到订阅成功不能覆盖 B 或清除 B 的订阅加载状态', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
      ...
      if (url.includes('/blog/channels/slug/') && url.includes('/collections')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/blog/posts?')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/feed/subscribe/channel/channel-1/status')) return Promise.resolve(response({ subscribed: false }))
      if (url.includes('/feed/subscribe/channel/channel-2/status')) return subscriptionStatusB.promise
      if (url.includes('/feed/subscribe/channel/channel-1') && init?.method === 'POST') return subscribeA.promise
      if (url.includes('/blog/bookmarks')) return Promise.resolve(response({ data: [] }))
      if (url.includes('/feed/reading-list')) return Promise.resolve(response({ items: [] }))
      throw new Error(`unexpected request: ${url}`)
    })
