...
- 双栏布局使用共享 token：主内容区占剩余空间，推荐区使用 `var(--a-recommendation-width)`（标准值 `20rem`），列间距使用 `var(--a-content-column-gap)`（标准值 `2rem`）。目标视觉比例约为主内容 `70%`、推荐区 `30%`。
...
2. `partial`：只覆盖推荐/附加内容区，宽度使用 `var(--a-recommendation-width)` 或由实际推荐栏锚点计算。
