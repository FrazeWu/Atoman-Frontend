...
let packageVersion: { version?: string } = {};
try {
...
export default defineConfig(({ mode }) => {
	...
	const objectStorageProxyTarget =
		env.VITE_DEV_OBJECT_STORAGE_PROXY_TARGET || "http://127.0.0.1:9100";
	const appVersion = normalizeVersion(
		env.VITE_APP_VERSION || packageVersion.version,
	);
	const apiProxy = {
