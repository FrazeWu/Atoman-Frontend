...
describe("markdown renderer style loading", () => {
  it("keeps highlight and katex styles out of the global app entry", () => {
    ...
    expect(markdownRendererSource).toMatch(
      /import\(\s*["']katex\/dist\/katex\.min\.css["']\s*\)/,
    );
    expect(markdownRendererSource).toMatch(
      /import\(\s*["']highlight\.js\/styles\/atom-one-dark\.css["']\s*\)/,
    );
  });
  ...
  it("lazy-loads heavy markdown runtime dependencies", () => {
    ...
    expect(markdownRendererSource).toMatch(
      /import\(\s*["']marked-highlight["']\s*\)/,
    );
    expect(markdownRendererSource).toMatch(
      /import\(\s*["']marked-katex-extension["']\s*\)/,
    );
    expect(markdownRendererSource).toMatch(
      /import\(\s*["']highlight\.js\/lib\/core["']\s*\)/,
    );
  });
});
