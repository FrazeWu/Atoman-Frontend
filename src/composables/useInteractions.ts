...
export function useInteractions(
	...
	const applyTargetState = (
		data: unknown,
		options: { applyCommentCount?: boolean } = {},
	) => {
		...
		if (typeof values.Liked === "boolean") liked.value = values.Liked;
		if (typeof values.viewer_liked === "boolean")
		...
		if (applyCommentCount && typeof values.CommentCount === "number")
			commentCount.value = values.CommentCount;
	};
	...
	const createComment = async (
		...
		try {
			const body = {
				...
				...(parentCommentId
					? {
							[moduleName === "forum" ? "reply_to_id" : "parent_comment_id"]:
								parentCommentId,
						}
					: {}),
				...(options?.timestamp_sec !== undefined
