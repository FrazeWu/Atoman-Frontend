...
const PButtonStub = {
  ...
  emits: ["click"],
  template:
    '<button type="button" v-bind="$attrs" :disabled="disabled || loading" @click="$emit(\'click\', $event)">{{ loading ? loadingText : label }}<slot /></button>',
};

const { navigateModuleWithShutter, routerPush, routerReplace, routeQuery } =
  vi.hoisted(() => ({
...
const feedViewStubs = {
  ...
  PClip: {
    ...
    emits: ["click"],
    template:
      '<button type="button" class="p-clip-stub" :title="title" :aria-label="ariaLabel" @click="$emit(\'click\')">{{ label }}<slot /></button>',
  },
  PShortcutHints: true,
...
describe("FeedView", () => {
  beforeEach(() => {
    ...
    feedStore.fetchSubscriptionRules = vi
      .fn()
    ...
    feedStore.createSubscriptionRule = vi
      .fn()
  ...
  it("merges duplicate stories by default and persists an explicit off state", async () => {
    ...
    const timelineCall = vi
      ...
      .mock.calls.find(([input]) => String(input).includes("/feed/timeline?"));
  ...
  it("keeps a single source timeline unmerged and hides the merge control", async () => {
    ...
    expect(wrapper.find('[data-test="feed-merge-duplicates"]').exists()).toBe(
      false,
    );
  });
  ...
  it("restores the explicit unmerged route state", async () => {
    ...
    expect(
      (
        wrapper.get('[data-test="feed-merge-duplicates"]')
  ...
  it("shows a new-content prompt and refreshes the first page with the current group filter", async () => {
    ...
    try {
      ...
      const timelineCalls = vi
        .mocked(globalThis.fetch)
        .mock.calls.map(([input]) => String(input))
        .filter(
          (url) =>
            url.includes("/feed/timeline?") && url.includes("group_id=group-1"),
        );
    ...
    try {
      ...
      expect(
        fetchMock.mock.calls.some(([input]) =>
          String(input).includes("/feed/timeline/updates"),
        ),
      ).toBe(false);
    } finally {
      vi.useRealTimers();
  ...
  it("checks for new content when the timeline returns to the foreground", async () => {
    ...
    fetchMock.mockImplementation(async (input) => {
      if (String(input).includes("/feed/timeline/updates")) {
        return new Response(
          JSON.stringify({
            data: { has_updates: false, checked_at: "2026-07-21T10:01:00Z" },
          }),
          { status: 200 },
        );
      }
  ...
  it("shows duplicate sources and marks the whole cluster read while saving only the primary", async () => {
    vi.mocked(globalThis.fetch).mockImplementation(async (input) => {
      if (String(input).includes("/feed/timeline?")) {
        return new Response(JSON.stringify(clusteredTimelineResponse()), {
          status: 200,
        });
      }
      return new Response(JSON.stringify({ data: [] }), { status: 200 });
    ...
    const toggleStar = vi
      .spyOn(feedStore, "toggleStar")
      .mockResolvedValue(undefined as any);
    const toggleReadingList = vi
    ...
    expect(wrapper.find('[data-test="feed-duplicate-sources"]').exists()).toBe(
      false,
    );
    await wrapper.get('[data-test="feed-duplicate-toggle"]').trigger("click");
    expect(
      wrapper.get('[data-test="feed-duplicate-sources"]').text(),
    ).toContain("Source A");
    expect(
      wrapper.get('[data-test="feed-duplicate-sources"]').text(),
    ).toContain("Source B");
  ...
  it("groups duplicate source details in a responsive summary block", async () => {
    ...
    expect(summary.find('[data-test="feed-duplicate-toggle"]').exists()).toBe(
      true,
    );
    await summary.get('[data-test="feed-duplicate-toggle"]').trigger("click");
    expect(
      summary.get('[data-test="feed-duplicate-sources"]').text(),
    ).toContain("Source B");
  });
  ...
  it("rolls a source article back to unread when marking fails", async () => {
    ...
    const wrapper = mount(FeedView, { global: { stubs: feedViewStubs } });
    await flushPromises();
    const sourceItem = JSON.parse(
      JSON.stringify((wrapper.vm as any).timeline[0]),
    );
  ...
  it("marks the whole cluster unread from the timeline read toggle", async () => {
    vi.mocked(globalThis.fetch).mockImplementation(async (input) => {
      if (String(input).includes("/feed/timeline?")) {
        return new Response(JSON.stringify(clusteredTimelineResponse(true)), {
          status: 200,
        });
      }
      return new Response(JSON.stringify({ data: [] }), { status: 200 });
    ...
    const feedStore = useFeedStore();
    const markUnread = vi
      .spyOn(feedStore, "markItemsUnread")
      .mockResolvedValue(true);
  ...
  it("marks the whole cluster read when podcast playback starts", async () => {
    ...
    (wrapper.vm as any).playFeedItemFromSheet(
      (wrapper.vm as any).timeline[0].feed_item,
    );
    await flushPromises();
  ...
  it("subscribes selected onboarding recommendations and completes after one success", async () => {
    ...
    const recommendations = [
      {
        ...
        title: "One",
        category: "blog",
        rss_url: "https://one.example/feed",
        cover_url: "",
      ...
      },
      {
        ...
        title: "Two",
        category: "blog",
        rss_url: "https://two.example/feed",
        cover_url: "",
        health_status: "healthy",
      },
    ];
    ...
    vi.spyOn(onboardingStore, "loadRecommendations").mockResolvedValue(
      undefined,
    );
    const completeSpy = vi
      .spyOn(onboardingStore, "complete")
      .mockResolvedValue(true);
    const subscribeSpy = vi
      .spyOn(feedStore, "subscribeToRSS")
      .mockImplementation(async (url: string) => url.includes("one"));
  ...
  it("opens internal posts in the feed article sheet", async () => {
    ...
    const wrapper = mount(FeedView, {
      global: {
        stubs: {
          ...
          PEmpty: {
            props: ["text"],
            template: '<div data-test="empty-state">{{ text }}</div>',
          },
          PPageHeader: {
            template: '<header><slot /><slot name="action" /></header>',
          },
    ...
    expect(
      fetchMock.mock.calls.some(([url]) =>
        String(url).includes("/feed/timeline/mark-read"),
      ),
    ).toBe(false);
    expect(wrapper.findComponent({ name: "FeedArticleSheet" }).exists()).toBe(
      true,
    );
  });
      ...
      global: {
        stubs: {
          ...
          SubscriptionAddSheet: {
            ...
            props: ["show", "top"],
            template:
              '<div data-test="add-sheet-probe" :data-show="String(show)" :data-top="top"></div>',
          },
        },
      },
    });
  ...
  it("shows channel source names for internal subscription posts and opens the source article sheet", async () => {
      ...
      global: {
        stubs: {
          ...
          FeedSourceArticlesSheet: {
            ...
            template:
              '<section data-test="source-sheet" :data-show="String(show)"><h2>{{ source?.title }}</h2><article v-for="item in items" :key="item.post?.id || item.feed_item?.id">{{ item.post?.title || item.feed_item?.title }}</article></section>',
          },
        },
      },
    });
    ...
    expect(wrapper.find(".feed-entry-meta").text()).toContain("Alice");
    ...
    expect(
      wrapper.get('[data-test="source-sheet"]').attributes("data-show"),
    ).toBe("true");
    expect(wrapper.get('[data-test="source-sheet"]').text()).toContain(
      "思想频道",
    );
    expect(wrapper.get('[data-test="source-sheet"]').text()).toContain(
      "内部文章",
      ...
      vi
        ...
        .mock.calls.some(([input]) =>
          String(input).includes(
            "/feed/timeline?limit=100&feed_source_id=source-rss-1",
          ),
        ),
    ).toBe(true);
  });
  ...
  it("shows current source context and returns to all subscriptions", async () => {
    ...
    expect(wrapper.get('[data-test="feed-current-source"]').text()).toContain(
      "Example RSS",
    );
    expect(wrapper.get('[data-test="feed-current-source"]').text()).toContain(
      "3",
  ...
  it("marks the current source read while in a source view", async () => {
    ...
    await wrapper
      .findAll("button")
      .find((button) => button.text() === "当前来源已读")!
      .trigger("click");
    await flushPromises();
  ...
  it("keeps source read state unchanged when marking the current source read fails", async () => {
    ...
    expect(
      wrapper
        .findAll("button")
        .some((button) => button.text() === "当前来源已读"),
    ).toBe(true);
  });
  ...
  it("keeps all-read state unchanged when marking all feed read fails", async () => {
    ...
    expect(feedStore.fetchSubscriptions).not.toHaveBeenCalled();
    expect(
      wrapper.findAll("button").some((button) => button.text() === "全部已读"),
    ).toBe(true);
  });
  ...
  it("auto-marks sources as read from subscription flags instead of local automation rules", async () => {
    ...
    const autoReadEntry = wrapper
      .findAll(".p-entry")
      .find((el) => el.text().includes("自动标记为已读的条目"));
  ...
  it("auto-adds sources to reading list from subscription flags instead of local automation rules", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining("/feed/reading-list"),
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ feed_item_id: "feed-item-auto-later-1" }),
      }),
    );
    expect(feedStore.readingListItemIds.has("feed-item-auto-later-1")).toBe(
      true,
    );
  ...
  it("filters the mixed timeline by source content type", async () => {
    ...
    const podcastFilter = wrapper.get(
    ...
    const internalFilter = wrapper.get(
  ...
  it("prefers the custom subscription title over the raw rss url in external feed items", async () => {
      ...
      global: {
        stubs: {
          ...
          FeedSourceArticlesSheet: {
            ...
            props: ["show", "source", "items"],
            template:
              '<section data-test="source-sheet" :data-show="String(show)"><h2>{{ source?.title }}</h2></section>',
          },
        },
      },
    });
    ...
    expect(wrapper.find(".feed-entry-meta").text()).not.toContain(
      "https://feeds.acast.com/public/shows/68004395b4ef799a7a410371",
    );
  ...
  it("falls back to the feed source name when the subscription title is just the rss url", async () => {
    ...
    expect(wrapper.find(".feed-entry-meta").text()).toContain(
      "Acast Show Name",
    );
    expect(wrapper.find(".feed-entry-meta").text()).not.toContain(
      "https://feeds.acast.com/public/shows/68004395b4ef799a7a410371",
  ...
  it("does not render the old right-edge subscription source drawer", async () => {
    ...
    expect(wrapper.findComponent({ name: "PIndexTrigger" }).exists()).toBe(
      false,
    );
    expect(
      wrapper.findComponent({ name: "SubscriptionIndexSheet" }).exists(),
    ).toBe(false);
  });
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            props: ["show"],
            template:
              '<div data-test="manage-sheet" :data-show="String(show)"></div>',
          },
          FeedArticleSheet: true,
        },
      },
  ...
  it("closes add subscription sheet when opening subscription management", async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionAddSheet: {
            ...
            props: ["show"],
            template:
              '<div data-test="add-sheet" :data-show="String(show)"></div>',
          },
          SubscriptionManageSheet: {
            name: "SubscriptionManageSheet",
            props: ["show"],
    ...
    expect(wrapper.get('[data-test="add-sheet"]').attributes("data-show")).toBe(
      "true",
    );
  ...
  it("shows manage errors when OPML import fails", async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            emits: ["import-opml"],
            template:
              '<section data-test="manage-sheet" @click="$emit(\'import-opml\', file)">{{ error }}</section>',
            setup() {
              return { file };
            },
          },
    ...
    expect(wrapper.get('[data-test="manage-sheet"]').text()).toContain(
      "导入失败",
    );
  });
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            emits: ["export-opml"],
            template:
              '<section data-test="manage-sheet" @click="$emit(\'export-opml\')">{{ error }}</section>',
          },
          FeedArticleSheet: true,
        },
      },
  ...
  it("shows manage errors when saving subscription rules fails", async () => {
    const feedStore = useFeedStore();
    vi.spyOn(feedStore, "createSubscriptionRule").mockImplementation(
      async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            emits: ["save-rule"],
            template:
              '<section data-test="manage-sheet" @click="$emit(\'save-rule\', rule)">{{ error }}</section>',
            setup() {
              return { rule };
            },
          },
  ...
  it.each([
    ...
    "shows manage errors when $eventName fails",
    async ({ eventName, methodName, errorText, args }) => {
      const feedStore = useFeedStore();
        ...
        global: {
          stubs: {
            ...
            SubscriptionManageSheet: {
              ...
              emits: [eventName],
              template:
                '<section data-test="manage-sheet" @click="$emit(eventName, ...args)">{{ error }}</section>',
              setup() {
  ...
  it("reloads the timeline after a subscription refresh succeeds", async () => {
    const feedStore = useFeedStore();
    const syncSubscription = vi
      .spyOn(feedStore, "syncSubscription")
      .mockResolvedValue({
    ...
    const wrapper = mount(FeedView, { global: { stubs: feedViewStubs } });
    await flushPromises();
    const timelineCallCount = () =>
      vi
        .mocked(globalThis.fetch)
        .mock.calls.filter(([input]) =>
          String(input).includes("/feed/timeline?"),
        ).length;
    const before = timelineCallCount();

    wrapper
      .findComponent({ name: "SubscriptionManageSheet" })
      .vm.$emit("sync-subscription", "sub-1");
    await flushPromises();
  ...
  it("reloads the timeline when a failed refresh still inserted items", async () => {
    ...
    expect(timelineCallCount()).toBeGreaterThan(before);
    expect(
      wrapper.findComponent({ name: "SubscriptionManageSheet" }).props("error"),
    ).toBe("failed to save refresh status");
  });
  ...
  it("reloads the timeline and reports partial refresh-all failures", async () => {
    ...
    wrapper
      .findComponent({ name: "SubscriptionManageSheet" })
      .vm.$emit("sync-all-subscriptions");
    await flushPromises();
  ...
  it.each([
    ...
    "shows manage errors when $eventName fails",
    async ({ eventName, methodName, errorText, payload }) => {
      const feedStore = useFeedStore();
      feedStore.subscriptionRules = [
        ...
        global: {
          stubs: {
            ...
            SubscriptionManageSheet: {
              ...
              emits: [eventName],
              template:
                '<section data-test="manage-sheet" @click="$emit(eventName, payload)">{{ error }}</section>',
              setup() {
      ...
      global: {
        stubs: {
          ...
          FeedTimelineFooter: {
            ...
            props: ["total"],
            template:
              '<div data-test="feed-footer" :data-total="String(total)"></div>',
          },
        },
      },
    });
  ...
  it("auto-adds subscriptions from the unified add sheet submit event", async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionAddSheet: {
            ...
            props: ["show"],
            template:
              "<div data-test=\"add-sheet\" @click=\"$emit('submit', { input: 'https://github.com/DIYgod/RSSHub', title: 'RSSHub Repo' })\"></div>",
          },
          SubscriptionManageSheet: true,
          FeedArticleSheet: true,
        },
    ...
    const feedStore = useFeedStore();
    const autoAddSpy = vi
      .spyOn(feedStore, "autoAddSubscription")
      .mockResolvedValue(true);
  ...
  it("plays the selected podcast from the article sheet", async () => {
    ...
    expect(createPodcastSongSpy).toHaveBeenCalledWith(
      expect.objectContaining({ id: "feed-item-play-1" }),
    );
  ...
  it("creates a rule then confirms and applies it to existing subscriptions", async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            template:
              "<section data-test=\"manage-sheet\" @click=\"$emit('save-rule', { id: null, payload: { name: '播客归档', enabled: true, match_type: 'source_category', conditions_json: { categories: ['podcast'] }, action_group_id: 'group-1', action_muted: false, action_auto_mark_read: true, action_auto_add_reading_list: false } })\" />",
          },
          FeedArticleSheet: true,
        },
      },
    ...
    expect(feedStore.applySubscriptionRules).toHaveBeenCalledWith({
      rule_id: "rule-1",
    });
  ...
  it("updates subscription flags from the management sheet", async () => {
    ...
    const updateSubscription = vi
      .spyOn(feedStore, "updateSubscription")
      .mockResolvedValue(true);
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            template:
              "<section data-test=\"manage-sheet\" @click=\"$emit('update-subscription', 'sub-1', { is_muted: true })\" />",
          },
          FeedArticleSheet: true,
        },
      },
  ...
  it("moves subscriptions back to the default group through the group endpoint", async () => {
    const feedStore = useFeedStore();
    const setSubscriptionGroup = vi
      .spyOn(feedStore, "setSubscriptionGroup")
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            template:
              "<section data-test=\"manage-sheet\" @click=\"$emit('move-subscription', 'sub-1', '')\" />",
          },
          FeedArticleSheet: true,
        },
      },
    ...
    expect(updateSubscription).not.toHaveBeenCalledWith("sub-1", {
      group_id: "",
    });
  });
  ...
  it("reorders and applies rules from the manage sheet", async () => {
      ...
      global: {
        stubs: {
          ...
          SubscriptionManageSheet: {
            ...
            template:
              '<section><button data-test="move-down" @click="$emit(\'move-rule-down\', \'rule-1\')" /><button data-test="apply-all" @click="$emit(\'apply-all-rules\')" /></section>',
          },
          FeedArticleSheet: true,
        },
      },
    ...
    expect(feedStore.reorderSubscriptionRules).toHaveBeenCalledWith([
      "rule-2",
      "rule-1",
    ]);
    expect(feedStore.applySubscriptionRules).toHaveBeenCalledWith({
