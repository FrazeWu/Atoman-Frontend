...
describe('borderless white UI contract', () => {
  ...
  it('uses icon-and-label sidebar items without archive numbering', () => {
    ...
    expect(item).toContain('border-color: transparent;')
    ...
    expect(globalStyle).toContain('.p-sidebar-item.active::before')
    expect(globalStyle).toContain('background-color: var(--a-color-fg);')
