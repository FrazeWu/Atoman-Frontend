...
async function responseData<T>(response: { ok: boolean; data: unknown }, fallback: string): Promise<T> {
  const payload = (response.data || {}) as { data?: T; error?: { message?: string } | string }
  if (!response.ok) {
...
async function loadAll() {
  ...
  try {
	const [groupResponse, categoryResponse, permissionResponse] = await Promise.all([
	  apiRequestResult(api.v1.forum.groups, { headers: headers() }),
	  apiRequestResult(api.v1.forum.categories, { headers: headers() }),
	  apiRequestResult(api.v1.forum.categoryPermissions, { headers: headers() }),
	])
...
async function saveGroup() {
  ...
  error.value = ''
  try {
	const response = await apiRequestResult(url, { method, headers: headers(true), body: JSON.stringify(payload) })
...
async function deleteGroup() {
  ...
  try {
	await responseData(await apiRequestResult(api.v1.forum.group(targetGroupId), { method: 'DELETE', headers: headers() }), '删除用户组失败')
...
async function addMember() {
  ...
  try {
	await responseData(await apiRequestResult(api.v1.forum.groupMember(targetGroupId, targetUserId), { method: 'PUT', headers: headers() }), '添加成员失败')
...
async function removeMember(userId: string) {
  if (!selectedGroup.value) return
	const targetGroupId = selectedGroup.value.id
  try {
	await responseData(await apiRequestResult(api.v1.forum.groupMember(targetGroupId, userId), { method: 'DELETE', headers: headers() }), '移除成员失败')
	const targetGroup = groups.value.find(group => group.id === targetGroupId)
...
async function savePermission() {
  ...
  try {
	const saved = await responseData<ForumCategoryPermission>(await apiRequestResult(api.v1.forum.categoryPermissions, { method: 'PUT', headers: headers(true), body: JSON.stringify(body) }), '保存权限失败')
...
async function deletePermission() {
  ...
  try {
	await responseData(await apiRequestResult(api.v1.forum.categoryPermission(targetPermissionId), { method: 'DELETE', headers: headers() }), '清除权限失败')
