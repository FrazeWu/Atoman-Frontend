...
describe("SiteFooter", () => {
	it("renders the confirmed two-level footbar content", () => {
		...
		expect(
			wrapper.get('[data-footer-action="docker-proxy"]').attributes("target"),
		).toBe("_blank");
		expect(
	...
	it("keeps footer sheets within the viewport above fixed bottom UI", () => {
		const sheetSource = readFileSync(
			resolve(
				__dirname,
				"../../../src/components/system/footer/SiteFooterSheet.vue",
			),
			"utf8",
		);

		expect(sheetSource).toContain(
			"calc(100dvh - var(--a-content-bottom-offset))",
		);
	});
	...
	it("uses the confirmed destinations for source code and feedback", async () => {
		...
		expect(
			wrapper.get('[data-footer-action="source"]').attributes("href"),
		).toBe("https://github.com/FrazeWu/Atoman");
