...
export function useMusicFavoritePlaylist() {
	...
	async function loadPlaylists() {
		...
		favoritePlaylistId.value = String(
			playlists.value.find((playlist) => playlist.kind === "favorite")?.id ||
				"",
		);
	...
	async function requireFavoritePlaylistId() {
		...
		if (!favoritePlaylistId.value)
			throw new Error("Favorite playlist is unavailable");
	...
	async function loadFavoriteSongs(
		...
		const loadedIds = new Set(
			await getMusicPlaylistSongStatus(playlistId, uniqueIds),
		);
