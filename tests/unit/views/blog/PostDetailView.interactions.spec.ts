...
describe('PostDetailView shared interactions', () => {
  ...
  beforeEach(() => {
    ...
    mocks.interactions.unlike.mockImplementation(async () => {
      ...
      mocks.interactions.likeCount.value = Math.max(0, mocks.interactions.likeCount.value - 1)
    })
    ...
    mocks.createContentConsumptionTracker.mockReset().mockImplementation((options) => {
  ...
  it('切到 B 后丢弃 A 的迟到主响应及其副作用', async () => {
    ...
    postB.resolve(response({ data: postDetail('post-2', '文章 B', { channel_id: 'channel-b', liked: true, likes_count: 8 }) }))
    await flushPromises()
    expect(wrapper.text()).toContain('文章 B')

    postA.resolve(response({ data: postDetail('post-1', '文章 A', { channel_id: 'channel-a', likes_count: 3 }) }))
    await flushPromises()
    ...
    expect(mocks.setPageMeta).toHaveBeenCalledWith(expect.objectContaining({ title: '文章 B' }))
    expect(mocks.recordEvent).toHaveBeenCalledTimes(1)
    expect(mocks.recordEvent).toHaveBeenCalledWith(expect.objectContaining({ content_id: 'post-2', source: 'timeline' }))
    const readEvents = vi.mocked(fetch).mock.calls.filter(([url]) => String(url).includes('/feed/events/read'))
    expect(readEvents).toHaveLength(1)
    expect(JSON.parse(String(readEvents[0][1]?.body))).toMatchObject({ source_id: 'channel-b' })
  })
  ...
  it('切换文章时立即清空 A 的文章、收藏和嵌入状态', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input) => {
      const url = String(input)
      if (url.includes('/blog/posts/post-1')) {
        return Promise.resolve(response({ data: postDetail('post-1', '文章 A', { content: `:::post{id="${embedID}"}\n:::` }) }))
      }
      if (url.includes(`/blog/posts/${embedID}`)) {
        return Promise.resolve(response({ data: postDetail(embedID, '嵌入文章') }))
      }
      if (url.includes('/blog/posts/post-2')) return postB.promise
      if (url.includes('/blog/bookmarks')) return Promise.resolve(response({ data: [{ post_id: 'post-1' }] }))
      return Promise.resolve(response({ data: [] }))
    }))
    ...
    const academicButton = wrapper.findAll('button').find((button) => button.text().includes('学术双栏'))
  ...
  it('切到 B 后丢弃 A 的迟到收藏操作结果', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input) => {
      const url = String(input)
      if (url.includes('/blog/posts/post-1')) return Promise.resolve(response({ data: postDetail('post-1', '文章 A') }))
      if (url.includes('/blog/posts/post-2')) return Promise.resolve(response({ data: postDetail('post-2', '文章 B') }))
      if (url.includes('/blog/bookmarks')) return Promise.resolve(response({ data: [] }))
      return Promise.resolve(response({ data: [] }))
    }))
    ...
    const togglePostBookmark = vi.spyOn(feedStore, 'togglePostBookmark').mockReturnValue(bookmarkToggle.promise)
    const bookmarkButton = wrapper.findAll('button').find((button) => button.text() === '收藏')
  ...
  it('文章从 A 切到 B 再切回 A 后丢弃首次 A 的迟到收藏操作结果', async () => {
    ...
    const { wrapper, router } = await mountPostDetailWithRouter()
    const feedStore = useFeedStore()
    vi.spyOn(feedStore, 'togglePostBookmark').mockReturnValue(bookmarkToggle.promise)
    const bookmarkButton = wrapper.findAll('button').find((button) => button.text() === '收藏')
  ...
  it('A 的迟到嵌入和收藏请求不能污染已加载的 B', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input) => {
      ...
      if (url.includes('/blog/bookmarks')) {
        bookmarkRequests += 1
        if (bookmarkRequests === 1) return Promise.resolve(response({ data: [{ post_id: 'post-2' }] }))
