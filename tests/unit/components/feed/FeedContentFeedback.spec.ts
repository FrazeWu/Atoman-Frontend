...
function mountFeedback(itemId = 'item-1') {
...
describe('FeedContentFeedback', () => {
  it('submits a quality issue and resets local state for the next article', async () => {
    ...
    const layoutButton = wrapper.findAll('[role="menuitem"]').find((button) => button.text().includes('排版错乱'))
    expect(layoutButton).toBeTruthy()

    await layoutButton!.trigger('click')
    await flushPromises()

    expect(apiRequestResult).toHaveBeenCalledWith('/api/v1/feed/items/item-1/content-feedback', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ kind: 'layout' }),
    }))
    expect(layoutButton!.attributes('disabled')).toBeDefined()

    await wrapper.setProps({ itemId: 'item-2' })
    await flushPromises()
    const nextLayoutButton = wrapper.findAll('[role="menuitem"]').find((button) => button.text().includes('排版错乱'))
