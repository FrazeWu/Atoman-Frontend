...
const shell = '<!doctype html><html><head><title>Atoman</title><meta data-default-meta name="description" content="old"><link data-default-meta rel="canonical" href="https://www.atoman.org/"></head><body></body></html>'
...
describe('public content SEO', () => {
  it('builds canonical metadata and structured data for a public music artist', async () => {
    ...
    expect(html).toContain('<title data-page-meta="content">艺术家 | 音乐档案 | Atoman</title>')
    expect(html).toContain('rel="canonical" href="https://www.atoman.org/music/artist/artist-1"')
    expect(html).toContain('"@type":"Person"')
    expect(html).not.toContain('content="old"')
  })

  it('marks a matched but unavailable detail route as noindex', async () => {
    const fetcher = vi.fn(async () => json({ error: 'not found' }, 404)) as unknown as typeof fetch
    const lookup = await resolvePublicContentSeo(
  ...
  it('does not noindex a valid route when its SEO source fails temporarily', async () => {
    const fetcher = vi.fn(async () => json({ error: 'unavailable' }, 503)) as unknown as typeof fetch
    const lookup = await resolvePublicContentSeo(
  ...
  it('collects public detail URLs while tolerating one failed module source', async () => {
    const fetcher = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('/music/artists')) {
        return json({ data: [{ id: 'artist-1', updated_at: '2026-08-01T00:00:00Z' }] })
      }
      ...
      if (url.includes('/debate/topics')) {
        return json({ data: [{ id: 'debate-1', updated_at: '2026-08-03T00:00:00Z' }] })
      }
      if (url.includes('/podcast/episodes')) return json([])
      if (url.includes('/videos')) {
        return json([
          { id: 'video-1', visibility: 'public', status: 'published', updated_at: '2026-08-04T00:00:00Z' },
          {
            id: 'video-2',
            visibility: 'private',
            status: 'published',
    ...
    const items = await collectPublicSitemapItems('https://api.atoman.org/api/v1', fetcher)
