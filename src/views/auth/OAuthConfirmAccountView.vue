  ...
  <div class="oauth-flow-page">
    <section class="oauth-flow-card" aria-labelledby="oauth-confirm-title">
      ...
      <template v-else>
        ...
        <form v-if="pending?.has_password" class="oauth-flow-form" @submit.prevent="submit">
          <PInput
            v-model="password"
            label="原账号密码"
            type="password"
        ...
        <div v-else-if="pending" class="oauth-flow-form">
          <p class="oauth-flow-status">请先使用已绑定的登录方式登录，或重置密码</p>
          <div class="oauth-flow-actions">
