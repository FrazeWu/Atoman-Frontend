...
async function searchUsers() { const generation = ++searchGeneration; searching.value = true; error.value = ''; try { const params = new URLSearchParams({ q: query.value.trim(), page_size: '20' }); const result = (await response<SearchUser[]>(await apiRequest(`${api.v1.forum.moderationUsers}?${params}`, { headers: headers() }), '搜索用户失败')).data || []; if (generation === searchGeneration) users.value = result } catch (cause) { if (generation === searchGeneration) error.value = cause instanceof Error ? cause.message : '搜索用户失败' } finally { if (generation === searchGeneration) searching.value = false } }
async function selectUser(user: SearchUser) { selected.value = user; actions.value = []; const id = user.uuid || ''; loadingActions.value = true; try { const result = await response<Action[]>(await apiRequest(`${api.v1.forum.userActions}?user_id=${id}&page=1&page_size=50`, { headers: headers() }), '加载处罚历史失败'); if (selected.value?.uuid === id) actions.value = result.data || [] } catch (cause) { if (selected.value?.uuid === id) error.value = cause instanceof Error ? cause.message : '加载处罚历史失败' } finally { if (selected.value?.uuid === id) loadingActions.value = false } }
