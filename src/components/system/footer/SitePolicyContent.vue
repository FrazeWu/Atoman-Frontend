...
const documents: Record<PolicyKind, PolicyDocument> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '2. 数据用途', paragraphs: ['我们使用这些数据完成身份验证、内容发布与展示、订阅与推荐、互动通知、私信、文件存储、服务安全、问题排查和站点访问分析。不会将数据用于与这些目的无关的营销。'] },
      {
        title: '3. 公开与非公开信息',
        paragraphs: [
      ...
      { title: '5. 服务提供方', paragraphs: ['为运行服务，必要数据可能由以下服务提供方处理：Cloudflare 提供页面托管、CDN、人机验证和对象存储；Neon 提供数据库；Resend 用于发送邮箱验证码；Google Analytics 用于统计页面访问、来源和站点使用情况。访问外部链接或播放第三方媒体时，对方也可能收到必要的网络请求信息。'] },
      {
        title: '6. 保存与注销',
        paragraphs: [
