...
describe('SiteFooter', () => {
  it('renders the confirmed two-level footbar content', () => {
    ...
    expect(wrapper.get('.site-footer-primary').text()).not.toContain('镜像加速')
    expect(wrapper.get('.site-footer-center').text()).toContain('镜像加速')
    expect(wrapper.get('[data-footer-action="docker-proxy"]').attributes('href')).toBe('https://proxy.atoman.org/')
    expect(wrapper.get('[data-footer-action="docker-proxy"]').attributes('target')).toBe('_blank')
    expect(wrapper.get('[data-footer-action="docker-proxy"]').attributes('rel')).toBe('noopener noreferrer')
