...
export function createApiClient(apiFetch: ApiFetcher) {
  ...
  async function apiRequestResult<T = any>(
    input: RequestInfo | URL,
    init?: RequestInit,
  ): Promise<ApiResult<T>> {
    const response = await apiRequest(input, init);
  ...
  async function unwrapResponseEnvelope<T, M = Record<string, unknown>>(
    response: Response,
  ...
  async function apiGetEnvelope<T, M = Record<string, unknown>>(
    url: string,
  ): Promise<ApiSuccess<T, M>> {
    return unwrapResponseEnvelope<T, M>(
      await apiFetch(url, {
        credentials: "include",
        headers: { Accept: "application/json" },
      }),
  ...
  async function apiRequestEnvelope<T, M = Record<string, unknown>>(
    url: string,
    init: RequestInit = {},
  ): Promise<ApiSuccess<T, M>> {
    return unwrapResponseEnvelope<T, M>(
