...
const props = withDefaults(defineProps<{ open: boolean; messageId: string; submitting?: boolean; error?: string }>(), { submitting: false, error: '' }); const emit = defineEmits<{ close: []; report: [payload: { messageId: string; reason: string; detail: string }] }>(); const reason = ref(''); const detail = ref(''); const submit = () => { if (reason.value && !props.submitting) emit('report', { messageId: props.messageId, reason: reason.value, detail: detail.value.trim() }) }
</script>
