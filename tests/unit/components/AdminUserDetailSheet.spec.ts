...
const stubs = {
  ...
  PConfirm: defineComponent({
    ...
    template: '<button v-if="show" data-test="confirm-revoke" @click="$emit(\'confirm\')">确认</button>',
  }),
}
...
describe('AdminUserDetailSheet', () => {
  beforeEach(() => {
    ...
    vi.mocked(listAdminUserLoginEvents).mockResolvedValue({
      data: [{
        id: 'event-id', session_id: 'session-id', method: 'password', result: 'succeeded',
        ip_address: '203.0.113.19', ip_prefix: '203.0.113.0/24', location: 'Berlin · DE',
        device_name: 'Mac 浏览器', user_agent: 'Mozilla/5.0', created_at: '2026-07-31T10:00:00Z',
      }],
      meta: { page: 1, page_size: 20, total: 1, has_more: false },
    })
    vi.mocked(listAdminUserSessions).mockResolvedValue([{
      id: 'session-id', kind: 'web', device_name: 'Mac 浏览器', user_agent: 'Mozilla/5.0',
      ip_address: '203.0.113.19', ip_prefix: '203.0.113.0/24', location: 'Berlin · DE',
      created_at: '2026-07-31T10:00:00Z', last_active_at: '2026-07-31T10:10:00Z',
    }])
  ...
  it('revokes a selected session and refreshes the detail', async () => {
    vi.mocked(listAdminUserSessions).mockResolvedValueOnce([{
      id: 'session-id', kind: 'web', device_name: 'Mac 浏览器', user_agent: 'Mozilla/5.0',
      ip_address: '203.0.113.19', ip_prefix: '203.0.113.0/24', location: 'Berlin · DE',
      created_at: '2026-07-31T10:00:00Z', last_active_at: '2026-07-31T10:10:00Z',
    ...
    await flushPromises()
    await wrapper.get('[data-test="revoke-session-session-id"]').trigger('click')
    await wrapper.get('[data-test="confirm-revoke"]').trigger('click')
    await flushPromises()

    expect(revokeAdminUserSession).toHaveBeenCalledWith('member-id', 'session-id')
